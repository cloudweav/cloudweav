...
func addAuthenticatedRoles(apply apply.Apply) error {
	return apply.
		WithDynamicLookup().
		WithSetID("cloudweav-authenticated").
		ApplyObjects(
			&rbacv1.RoleBinding{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "cloudweav-public",
			...
			&rbacv1.ClusterRole{
				ObjectMeta: metav1.ObjectMeta{
					Name: "cloudweav-authenticated",
				},
				Rules: []rbacv1.PolicyRule{
					{
						Verbs:         []string{"get", "watch"},
						APIGroups:     []string{"cloudweavhci.io"},
						Resources:     []string{"settings"},
						ResourceNames: settings.WhiteListedSettings,
					},
					{
						Verbs:     []string{"get", "list", "watch"},
						APIGroups: []string{"network.cloudweavhci.io"},
