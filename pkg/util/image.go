...
func backingImageLegacyName(image *cloudweavv1.VirtualMachineImage) string {
	return fmt.Sprintf("%s-%s", image.Namespace, image.Name)
}

func backingImageLegacyNameV2(image *cloudweavv1.VirtualMachineImage) string {
	return lhutil.AutoCorrectName(backingImageLegacyName(image), lhdatastore.NameMaximumLength)
}

func backingImageName(image *cloudweavv1.VirtualMachineImage) string {
...
func GetImageStorageClassParameters(backingImageCache ctllhv1.BackingImageCache, image *cloudweavv1.VirtualMachineImage) (map[string]string, error) {
	...
	if image.Spec.SourceType == cloudweavv1.VirtualMachineImageSourceTypeClone && image.Spec.SecurityParameters.CryptoOperation == cloudweavv1.VirtualMachineImageCryptoOperationTypeEncrypt {
