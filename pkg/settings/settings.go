...
var (
	...
	APIUIVersion                           = NewSetting("api-ui-version", "1.1.9") // Please update the CLOUDWEAV_API_UI_VERSION in package/Dockerfile when updating the version here.
	...
	UIPath                                 = NewSetting(UIPathSettingName, "/usr/share/cloudweav/cloudweav")
	...
	UpgradeCheckerURL                      = NewSetting("upgrade-checker-url", "https://cloudweav-upgrade-responder.rancher.io/v1/checkupgrade")
	ReleaseDownloadURL                     = NewSetting("release-download-url", "https://releases.rancher.com/cloudweav")
	...
	// CloudweavCSICCMVersion this is the chart version from https://github.com/cloudweav/charts instead of image versions
	CloudweavCSICCMVersion = NewSetting(CloudweavCSICCMSettingName, `{"cloudweav-cloud-provider":">=0.0.1 <0.3.0","cloudweav-csi-provider":">=0.0.1 <0.3.0"}`)
	NTPServers             = NewSetting(NTPServersSettingName, "")
	WhiteListedSettings    = []string{"server-version", "default-storage-class", "cloudweav-csi-ccm-versions", "default-vm-termination-grace-period-seconds"}
...
const (
	...
	DefaultDashboardUIURL                             = "https://releases.rancher.com/cloudweav-ui/dashboard/latest/index.html"
	...
	DefaultUIPluginURL                                = "https://releases.rancher.com/cloudweav-ui/plugin/cloudweav-latest/cloudweav-latest.umd.min.js"
	ContainerdRegistrySettingName                     = "containerd-registry"
	CloudweavCSICCMSettingName                        = "cloudweav-csi-ccm-versions"
...
func GetEnvKey(key string) string {
	return "CLOUDWEAV_" + strings.ToUpper(strings.Replace(key, "-", "_", -1))
}

func GetEnvDefaultValueKey(key string) string {
	return "CLOUDWEAV_" + strings.ToUpper(strings.Replace(key, "-", "_", -1)) + "_DEFAULT_VALUE"
}
