...
// fix https://github.com/cloudweav/cloudweav/issues/3681
...
func (c *nodeVolumeDetachController) isInUpgradePreDraining(node *corev1.Node) (bool, error) {
	...
	upgrades, err := c.upgradeCache.List(util.CloudweavSystemNamespaceName, labels.NewSelector())
	if err != nil {
		return false, err
	}
	for _, upgrade := range upgrades {
		if !cloudweavv1.UpgradeCompleted.IsTrue(upgrade) && !cloudweavv1.UpgradeCompleted.IsFalse(upgrade) {
			return true, nil
		}
	}
	return false, nil
...
// triggered by command: kubectl taint node $CLOUDWEAV_UPGRADE_NODE_NAME --overwrite kubevirt.io/drain=draining:NoSchedule
