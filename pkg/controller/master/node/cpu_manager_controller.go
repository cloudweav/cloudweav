...
import (
	...
	"github.com/cloudweav/cloudweav/pkg/util/catalog"
)
...
const (
	...
	CPUManagerScriptMountPath       string = "/cloudweav-helpers"
...
func (h *cpuManagerNodeHandler) OnNodeChanged(_ string, node *corev1.Node) (*corev1.Node, error) {
	...
	if _, found := node.Labels[CloudweavWitnessNodeLabelKey]; found {
		return node, nil
	}
...
func (h *cpuManagerNodeHandler) submitJob(updateStatus *CPUManagerUpdateStatus, node *corev1.Node) (*batchv1.Job, error) {
	image, err := catalog.FetchAppChartImage(h.appCache, h.namespace, releaseAppCloudweavName, []string{"generalJob", "image"})
	if err != nil {
