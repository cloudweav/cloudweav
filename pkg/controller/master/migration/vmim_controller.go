...
import (
	...
	rqutils "github.com/cloudweav/cloudweav/pkg/util/resourcequota"
)
...
func (h *Handler) OnVmimChanged(_ string, vmim *kubevirtv1.VirtualMachineInstanceMigration) (*kubevirtv1.VirtualMachineInstanceMigration, error) {
	...
	if vmim.Status.Phase == kubevirtv1.MigrationPending && !abortRequested {
		...
		if err := h.resetCloudweavMigrationStateInVmiAndSyncVM(vmi); err != nil {
...
// https://github.com/cloudweav/cloudweav/issues/6193
...
func (h *Handler) resetCloudweavMigrationStateInVmiAndSyncVM(vmi *kubevirtv1.VirtualMachineInstance) error {
	if err := h.resetCloudweavMigrationStateInVMI(vmi); err != nil {
