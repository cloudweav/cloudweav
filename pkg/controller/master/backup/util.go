...
const (
	// UI stores the mapping between access credential secret and ssh keys in the annotation `cloudweavhci.io/dynamic-ssh-key-names`
	// example: '{"secretname1":["sshkeyname1","sshkeyname2"],"secretname2":["sshkeyname3","sshkeyname4"]}'
	dynamicSSHKeyNamesAnnotation = "cloudweavhci.io/dynamic-ssh-key-names"
)
...
func isBackupMissingStatus(backup *cloudweavv1.VirtualMachineBackup) bool {
	return backup.Status == nil || backup.Status.SourceSpec == nil || backup.Status.VolumeBackups == nil
}

func isBackupTargetOnAnnotation(backup *cloudweavv1.VirtualMachineBackup) bool {
...
func isVMRestoreProgressing(vmRestore *cloudweavv1.VirtualMachineRestore) bool {
	return vmRestore.Status == nil || vmRestore.Status.Complete == nil || !*vmRestore.Status.Complete
}

func isVMRestoreMissingVolumes(vmRestore *cloudweavv1.VirtualMachineRestore) bool {
...
func IsNewVMOrHasRetainPolicy(vmRestore *cloudweavv1.VirtualMachineRestore) bool {
	return vmRestore.Spec.NewVM || vmRestore.Spec.DeletionPolicy == cloudweavv1.VirtualMachineRestoreRetain
}
...
func newReadyCondition(status corev1.ConditionStatus, reason string, message string) cloudweavv1.Condition {
	return cloudweavv1.Condition{
		Type:               cloudweavv1.BackupConditionReady,
		Status:             status,
		Message:            message,
		Reason:             reason,
...
func newProgressingCondition(status corev1.ConditionStatus, reason string, message string) cloudweavv1.Condition {
	return cloudweavv1.Condition{
		Type:   cloudweavv1.BackupConditionProgressing,
		Status: status,
...
func updateBackupCondition(ss *cloudweavv1.VirtualMachineBackup, c cloudweavv1.Condition) {
	ss.Status.Conditions = updateCondition(ss.Status.Conditions, c)
}

func updateCondition(conditions []cloudweavv1.Condition, c cloudweavv1.Condition) []cloudweavv1.Condition {
	found := false
...
func translateError(e *snapshotv1.VolumeSnapshotError) *cloudweavv1.Error {
	if e == nil {
		return nil
	}

	return &cloudweavv1.Error{
		Message: e.Message,
		Time:    e.Time,
	}
}
...
func getRestoreID(vmRestore *cloudweavv1.VirtualMachineRestore) string {
	return fmt.Sprintf("%s-%s", vmRestore.Name, vmRestore.UID)
}

func updateRestoreCondition(r *cloudweavv1.VirtualMachineRestore, c cloudweavv1.Condition) {
	r.Status.Conditions = updateCondition(r.Status.Conditions, c)
}

func getNewVolumes(vm *kubevirtv1.VirtualMachineSpec, vmRestore *cloudweavv1.VirtualMachineRestore) ([]kubevirtv1.Volume, error) {
...
func getRestorePVCName(vmRestore *cloudweavv1.VirtualMachineRestore, name string) string {
...
func sanitizeVirtualMachineAnnotationsForRestore(restore *cloudweavv1.VirtualMachineRestore, annotations map[string]string) (map[string]string, error) {
...
func sanitizeVirtualMachineForRestore(restore *cloudweavv1.VirtualMachineRestore, spec kubevirtv1.VirtualMachineInstanceSpec) kubevirtv1.VirtualMachineInstanceSpec {
