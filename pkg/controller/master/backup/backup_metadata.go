...
const (
	vmBackupMetadataFolderPath   = "cloudweav/vmbackups/"
	backupMetadataControllerName = "cloudweav-backup-metadata-controller"
)
...
type MetadataHandler struct {
	...
	settings             ctlcloudweavv1.SettingController
...
func (h *MetadataHandler) createVMImageIfNotExist(imageMetadata VirtualMachineImageMetadata) error {
	...
	if _, err := h.vmImages.Create(&cloudweavv1.VirtualMachineImage{
		ObjectMeta: metav1.ObjectMeta{
			Name:      imageMetadata.Name,
			Namespace: imageMetadata.Namespace,
		},
		Spec: cloudweavv1.VirtualMachineImageSpec{
			SourceType:             cloudweavv1.VirtualMachineImageSourceTypeRestore,
...
func (h *MetadataHandler) createVMBackupIfNotExist(backupMetadata VirtualMachineBackupMetadata, target *settings.BackupTarget) error {
	...
	if _, err := h.vmBackups.Create(&cloudweavv1.VirtualMachineBackup{
		ObjectMeta: metav1.ObjectMeta{
			Name:      backupMetadata.Name,
			Namespace: backupMetadata.Namespace,
		},
		...
		Status: &cloudweavv1.VirtualMachineBackupStatus{
			...
			BackupTarget: &cloudweavv1.BackupTarget{
