...
import (
	...
	ctlmonitoringv1 "github.com/cloudweav/cloudweav/pkg/generated/controllers/monitoring.coreos.com/v1"
	"github.com/cloudweav/cloudweav/pkg/settings"
	"github.com/cloudweav/cloudweav/pkg/util"
)

const (
	ControllerName                  = "cloudweav-storage-network-controller"
	StorageNetworkAnnotation        = "storage-network.settings.cloudweavhci.io"
	...
	StorageNetworkNetAttachDefNamespace = "cloudweav-system"
...
func (h *Handler) setConfiguredCondition(setting *cloudweavv1.Setting, finish bool, reason string, msg string) (*cloudweavv1.Setting, error) {
	...
	cloudweavv1.SettingConfigured.Reason(settingCopy, reason)
	cloudweavv1.SettingConfigured.Message(settingCopy, msg)
...
func (h *Handler) OnStorageNetworkChange(_ string, setting *cloudweavv1.Setting) (*cloudweavv1.Setting, error) {
	...
	var (
		updatedSetting *cloudweavv1.Setting
		err            error
		value          string
	)
...
func (h *Handler) checkIsSameHashValue(setting *cloudweavv1.Setting) bool {
	currentHash := h.sha1(setting.Value)
	savedHash := setting.Annotations[HashStorageNetworkAnnotation]
	return currentHash == savedHash
}

func (h *Handler) setHashAnnotations(setting *cloudweavv1.Setting) *cloudweavv1.Setting {
	setting.Annotations[HashStorageNetworkAnnotation] = h.sha1(setting.Value)
	return setting
}

func (h *Handler) setNadAnnotations(setting *cloudweavv1.Setting, newNad string) *cloudweavv1.Setting {
...
func (h *Handler) createNad(setting *cloudweavv1.Setting) (*nadv1.NetworkAttachmentDefinition, error) {
	var config Config
...
func (h *Handler) findOrCreateNad(setting *cloudweavv1.Setting) (*nadv1.NetworkAttachmentDefinition, error) {
...
func (h *Handler) checkValueIsChanged(setting *cloudweavv1.Setting) (*cloudweavv1.Setting, error) {
	var updatedSetting *cloudweavv1.Setting
	var err error
	nadAnnotation := ""
...
func (h *Handler) removeOldNad(setting *cloudweavv1.Setting) error {
...
func (h *Handler) handleLonghornSettingPostConfig(setting *cloudweavv1.Setting) (*cloudweavv1.Setting, error) {
...
func (h *Handler) checkVMImportControllerStatusAndStart() error {
	// check deployment cloudweav-system/cloudweav-cloudweav-vm-import-controller replica
	vmImportControllerDeploy, err := h.deploymentCache.Get(util.CloudweavSystemNamespaceName, util.CloudweavVMImportController)
	if err != nil {
		if apierrors.IsNotFound(err) {
...
func (h *Handler) checkVMImportControllerStatusAndStop() error {
	// check deployment cloudweav-system/cloudweav-cloudweav-vm-import-controller replica
	vmimportcontroller, err := h.deploymentCache.Get(util.CloudweavSystemNamespaceName, util.CloudweavVMImportController)
	if err != nil {
		if apierrors.IsNotFound(err) {
