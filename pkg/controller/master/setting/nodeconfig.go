...
func (h *Handler) syncNodeConfig(setting *cloudweavv1.Setting) error {
...
func (h *Handler) nodeOnChanged(_ string, node *corev1.Node) (*corev1.Node, error) {
	...
	if _, found := node.Labels[ctlnode.CloudweavWitnessNodeLabelKey]; found {
	...
	nodeConfig, err := h.nodeConfigs.Get(util.CloudweavSystemNamespaceName, node.Name, metav1.GetOptions{})
	if err != nil {
