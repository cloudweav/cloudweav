...
func (h *Handler) syncContainerdRegistry(setting *cloudweavv1.Setting) error {
	if setting.Value == "" {
...
func (h *Handler) removeCredentialInSetting(setting *cloudweavv1.Setting, registry *containerd.Registry) error {
...
func (h *Handler) syncContainerdRegistryConfigSecret(prevConfs, newConfs map[string]*corev1.Secret, host string, config containerd.RegistryConfig) (rkev1.RegistryConfig, error) {
	...
	if s, ok := prevConfs[host]; ok {
		...
		var err error
		authSecretName, err = genSecretName("cloudweav-containerd-registry-", host)
		if err != nil {
