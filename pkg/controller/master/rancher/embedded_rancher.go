...
func (h *Handler) RancherSettingOnChange(_ string, setting *rancherv3api.Setting) (*rancherv3api.Setting, error) {
	...
	if setting.Name == systemNamespacesSetting && !strings.Contains(setting.Default, "cloudweav-system") {
...
func (h *Handler) initializeSystemNamespaces(setting *rancherv3api.Setting) (*rancherv3api.Setting, error) {
	...
	cloudweavDefaultSystemNamespaces := strings.Join([]string{
	...
	logrus.Debugf("Updating system-namespaces setting to %s", cloudweavDefaultSystemNamespaces)
	toUpdate := setting.DeepCopy()
	toUpdate.Default = cloudweavDefaultSystemNamespaces
	return h.RancherSettings.Update(toUpdate)
}

// syncCACert updates the cacerts setting to ca certificate of Cloudweav setting
