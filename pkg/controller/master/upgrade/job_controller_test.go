		...
		},
		{
			...
			given: input{
				...
				job: newJobBuilder(testJobName).
					WithLabel(cloudweavUpgradeLabel, "test-upgrade-old").
					WithLabel(cloudweavUpgradeComponentLabel, manifestComponent).
					Completed().Build(),
				plan:    newTestPlanBuilder().Build(),
				upgrade: newTestUpgradeBuilder().WithLabel(cloudweavLatestUpgradeLabel, "true").ChartUpgradeStatus(v1.ConditionUnknown, "", "").Build(),
			},
			expected: output{
				job: newJobBuilder(testJobName).
					WithLabel(cloudweavUpgradeLabel, "test-upgrade-old").
