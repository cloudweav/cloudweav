...
func newTestChartJobBuilder() *jobBuilder {
	return newJobBuilder(testJobName).
		WithLabel(cloudweavUpgradeLabel, testUpgradeName).
		WithLabel(cloudweavUpgradeComponentLabel, manifestComponent)
}

func newTestUpgradeBuilder() *upgradeBuilder {
	return newUpgradeBuilder(testUpgradeName).
		WithLabel(cloudweavLatestUpgradeLabel, "true").
		Version(testVersion)
}

func newTestExistingVirtualMachineImage(namespace, name string) *cloudweavv1.VirtualMachineImage {
	return &cloudweavv1.VirtualMachineImage{
...
func newTestUpgradeLog() *cloudweavv1.UpgradeLog {
...
func newTestVirtualMachineImage() *cloudweavv1.VirtualMachineImage {
	return &cloudweavv1.VirtualMachineImage{
...
func TestUpgradeHandler_OnChanged(t *testing.T) {
	type input struct {
		...
		vmi     *cloudweavv1.VirtualMachineImage
		nodes   []*v1.Node
	}
	type output struct {
	...
	for _, tc := range testCases {
		...
		var handler = &upgradeHandler{
			...
			versionCache:     fakeclients.VersionCache(clientset.CloudweavhciV1beta1().Versions),
			vmClient:         fakeclients.VirtualMachineClient(clientset.KubevirtV1().VirtualMachines),
			vmImageClient:    fakeclients.VirtualMachineImageClient(clientset.CloudweavhciV1beta1().VirtualMachineImages),
			vmImageCache:     fakeclients.VirtualMachineImageCache(clientset.CloudweavhciV1beta1().VirtualMachineImages),
		}
		var actual output
...
func fakeImageExist(imageCache ctlcloudweavv1.VirtualMachineImageCache, displayName string) (bool, error) {
