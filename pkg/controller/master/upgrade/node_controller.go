...
// We label `cloudweavhci.io/pendingOSImage` to new OS version before rebooting a node.
// If `cloudweavhci.io/pendingOSImage` is equals to the value of `cloudweavhci.io/pendingOSImage`, we know the
// the reboot is done.
...
func (h *nodeHandler) OnChanged(_ string, node *corev1.Node) (*corev1.Node, error) {
	...
	expectedVersion, ok := node.Annotations[cloudweavNodePendingOSImage]
	if !ok {
		return node, nil
	}
	...
	if expectedVersion == node.Status.NodeInfo.OSImage {
		...
		err := h.retryUpdateNodeOnConflict(node.Name, func(n *corev1.Node) {
			...
			delete(n.Annotations, cloudweavNodePendingOSImage)
		})
		if err != nil {
			return nil, err
		}
