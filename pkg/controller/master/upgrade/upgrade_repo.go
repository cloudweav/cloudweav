...
stages:
  rootfs:
  - commands:
    - echo > /sysroot/cloudweav-serve-iso
`
...
func (r *Repo) CreateImageFromISO(isoURL string, checksum string) (*cloudweavv1.VirtualMachineImage, error) {
	imageSpec := &cloudweavv1.VirtualMachineImage{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: cloudweavSystemNamespace,
			Name:      r.upgrade.Name,
			Labels: map[string]string{
				cloudweavUpgradeLabel: r.upgrade.Name,
			},
...
func (r *Repo) createVM(image *cloudweavv1.VirtualMachineImage) (*kubevirtv1.VirtualMachine, error) {
	vmName := r.getVMName()
	vmRun := true
					...
					ReadinessProbe: &kubevirtv1.Probe{
						Handler: kubevirtv1.Handler{
							HTTPGet: &corev1.HTTPGetAction{
								Path: "/cloudweav-iso/cloudweav-release.yaml",
...
func (r *Repo) getInfo() (*repoinfo.RepoInfo, error) {
	releaseURL := fmt.Sprintf("http://%s.%s/cloudweav-iso/cloudweav-release.yaml", r.getRepoServiceName(), upgradeNamespace)
...
func (r *Repo) getImageList(version string, imageList map[string]bool) error {
	imageListURL := fmt.Sprintf("http://%s.%s/cloudweav-iso/bundle/cloudweav/images-lists-archive/%s/image_list_all.txt",
