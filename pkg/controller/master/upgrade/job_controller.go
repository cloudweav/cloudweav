...
import (
	...
	ctlclusterv1 "github.com/cloudweav/cloudweav/pkg/generated/controllers/cluster.x-k8s.io/v1beta1"
...
const (
	...
	upgradeStateLabel               = "cloudweavhci.io/upgradeState"
	upgradeJobTypeLabel             = "cloudweavhci.io/upgradeJobType"
...
func (h *jobHandler) OnChanged(_ string, job *batchv1.Job) (*batchv1.Job, error) {
	...
	case job.Labels[cloudweavUpgradeComponentLabel] == nodeComponent:
		return h.syncNodeJob(job)
	case job.Labels[cloudweavUpgradeComponentLabel] == manifestComponent:
...
func (h *jobHandler) syncNodeJob(job *batchv1.Job) (*batchv1.Job, error) {
	...
	nodeName, ok := job.Labels[cloudweavNodeLabel]
	if !ok {
		return job, nil
	}
	...
	upgradeName, ok := job.Labels[cloudweavUpgradeLabel]
	if !ok {
		return job, nil
	}
...
func (h *jobHandler) syncManifestJob(job *batchv1.Job) (*batchv1.Job, error) {
	...
	if upgrade.Labels[cloudweavLatestUpgradeLabel] != "true" {
		return job, nil
	}

	if !cloudweavv1.SystemServicesUpgraded.IsUnknown(upgrade) || job.Status.Active > 0 {
		return job, nil
	}
...
func (h *jobHandler) setNodeWaitRebootLabel(node *v1.Node, repoInfo *repoinfo.RepoInfo) error {
	...
	nodeUpdate.Annotations[cloudweavNodePendingOSImage] = repoInfo.Release.OS
