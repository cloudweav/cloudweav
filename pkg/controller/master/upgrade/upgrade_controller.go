...
import (
	...
	kubevirtctrl "github.com/cloudweav/cloudweav/pkg/generated/controllers/kubevirt.io/v1"
	...
	"github.com/cloudweav/cloudweav/pkg/upgradehelper/versionguard"
...
const (
	...
	upgradeNamespace               = "cloudweav-system"
	sucNamespace                   = "cattle-system"
	upgradeServiceAccount          = "system-upgrade-controller"
	cloudweavSystemNamespace       = "cloudweav-system"
	cloudweavUpgradeLabel          = "cloudweavhci.io/upgrade"
	cloudweavManagedLabel          = "cloudweavhci.io/managed"
	cloudweavLatestUpgradeLabel    = "cloudweavhci.io/latestUpgrade"
	cloudweavUpgradeComponentLabel = "cloudweavhci.io/upgradeComponent"
	cloudweavNodeLabel             = "cloudweavhci.io/node"
	upgradeImageRepository         = "panmeta/cloudweav-upgrade"

	cloudweavNodePendingOSImage = "cloudweavhci.io/pendingOSImage"

	preDrainAnnotation  = "cloudweavhci.io/pre-hook"
	postDrainAnnotation = "cloudweavhci.io/post-hook"
	...
	replicaReplenishmentAnnotation           = "cloudweavhci.io/" + replicaReplenishmentWaitIntervalSetting
	extendedReplicaReplenishmentWaitInterval = 1800

	imageCleanupPlanCompletedAnnotation = "cloudweavhci.io/image-cleanup-plan-completed"
	skipVersionCheckAnnotation          = "cloudweavhci.io/skip-version-check"
	defaultImagePreloadConcurrency      = 1

	preUpgradeVMsAnnotation = "cloudweavhci.io/pre-upgrade-vms"
...
type upgradeHandler struct {
	...
	upgradeController ctlcloudweavv1.UpgradeController
	upgradeLogClient  ctlcloudweavv1.UpgradeLogClient
	upgradeLogCache   ctlcloudweavv1.UpgradeLogCache
	versionCache      ctlcloudweavv1.VersionCache
	planClient        upgradectlv1.PlanClient
	planCache         upgradectlv1.PlanCache

	vmImageClient ctlcloudweavv1.VirtualMachineImageClient
...
func (h *upgradeHandler) OnChanged(_ string, upgrade *cloudweavv1.Upgrade) (*cloudweavv1.Upgrade, error) {
	...
	if cloudweavv1.UpgradeCompleted.GetStatus(upgrade) == "" {
		...
		cloudweavv1.LogReady.CreateUnknownIfNotExists(toUpdate)
		return h.upgradeClient.Update(toUpdate)
	}

	if (cloudweavv1.LogReady.IsTrue(upgrade) || cloudweavv1.LogReady.IsFalse(upgrade)) && cloudweavv1.ImageReady.GetStatus(upgrade) == "" {
		...
		if upgrade.Spec.Image == "" {
			...
			if err != nil && apierrors.IsAlreadyExists(err) {
				image, err = h.vmImageClient.Get(cloudweavSystemNamespace, upgrade.Name, metav1.GetOptions{})
				if err != nil {
					setUpgradeCompletedCondition(toUpdate, StateFailed, corev1.ConditionFalse, err.Error(), "")
					return h.upgradeClient.Update(toUpdate)
				}
			...
			imageUpdate.Labels[cloudweavUpgradeLabel] = upgrade.Name
			if _, err := h.vmImageClient.Update(imageUpdate); err != nil {
				return nil, err
			}
		}
		cloudweavv1.ImageReady.CreateUnknownIfNotExists(toUpdate)
		return h.upgradeClient.Update(toUpdate)
	}
	...
	if upgrade.Labels == nil || upgrade.Labels[cloudweavLatestUpgradeLabel] != "true" {
		return upgrade, nil
	}

	// clean upgrade repo VMs and images if a upgrade succeeds.
	if cloudweavv1.UpgradeCompleted.IsTrue(upgrade) {
		// try to clean up images before purging the repo VM
		_, exists := upgrade.Annotations[imageCleanupPlanCompletedAnnotation]
		if exists {
			return nil, h.cleanup(upgrade, cloudweavv1.UpgradeCompleted.IsTrue(upgrade))
		}
	...
	// upgrade failed
	if cloudweavv1.UpgradeCompleted.IsFalse(upgrade) {
	...
	if cloudweavv1.ImageReady.IsTrue(upgrade) && cloudweavv1.RepoProvisioned.GetStatus(upgrade) == "" {
		...
		cloudweavv1.RepoProvisioned.CreateUnknownIfNotExists(toUpdate)
		return h.upgradeClient.Update(toUpdate)
	}

	if cloudweavv1.RepoProvisioned.IsTrue(upgrade) && cloudweavv1.NodesPrepared.GetStatus(upgrade) == "" {
		toUpdate := upgrade.DeepCopy()
	...
	if cloudweavv1.NodesPrepared.IsTrue(upgrade) && cloudweavv1.SystemServicesUpgraded.GetStatus(upgrade) == "" {
		toUpdate := upgrade.DeepCopy()
	...
	if cloudweavv1.SystemServicesUpgraded.IsTrue(upgrade) && cloudweavv1.NodesUpgraded.GetStatus(upgrade) == "" {
		...
		cloudweavv1.NodesUpgraded.CreateUnknownIfNotExists(toUpdate)
		return h.upgradeClient.Update(toUpdate)
	}
...
func (h *upgradeHandler) OnRemove(_ string, upgrade *cloudweavv1.Upgrade) (*cloudweavv1.Upgrade, error) {
	if upgrade == nil {
		return nil, nil
	}
...
func (h *upgradeHandler) cleanupImages(upgrade *cloudweavv1.Upgrade, repo *Repo) error {
...
func (h *upgradeHandler) cleanup(upgrade *cloudweavv1.Upgrade, cleanJobs bool) error {
	...
	sets := labels.Set{
		cloudweavUpgradeLabel: upgrade.Name,
	}
	...
	if cloudweavv1.LogReady.IsTrue(upgrade) && upgrade.Status.UpgradeLog != "" {
		...
		cloudweavv1.UpgradeEnded.SetStatus(upgradeLogToUpdate, string(corev1.ConditionTrue))
		cloudweavv1.UpgradeEnded.Reason(upgradeLogToUpdate, "")
		cloudweavv1.UpgradeEnded.Message(upgradeLogToUpdate, "")
...
func initStatus(upgrade *cloudweavv1.Upgrade) {
	cloudweavv1.UpgradeCompleted.CreateUnknownIfNotExists(upgrade)
	if upgrade.Labels == nil {
		upgrade.Labels = make(map[string]string)
	}
	upgrade.Labels[upgradeStateLabel] = StatePreparingLoggingInfra
	upgrade.Labels[cloudweavLatestUpgradeLabel] = "true"
...
func (h *upgradeHandler) resetLatestUpgradeLabel(latestUpgradeName string) error {
	sets := labels.Set{
		cloudweavLatestUpgradeLabel: "true",
	}
	...
	for _, upgrade := range upgrades {
		...
		toUpdate := upgrade.DeepCopy()
		delete(toUpdate.Labels, cloudweavLatestUpgradeLabel)
...
func ensureSingleUpgrade(namespace string, upgradeCache ctlcloudweavv1.UpgradeCache) (*cloudweavv1.Upgrade, error) {
	sets := labels.Set{
		cloudweavLatestUpgradeLabel: "true",
	}
...
func getCachedRepoInfo(upgrade *cloudweavv1.Upgrade) (*repoinfo.RepoInfo, error) {
...
func upgradeEligibilityCheck(upgrade *cloudweavv1.Upgrade) (bool, string) {
...
func (h *upgradeHandler) prepareNodesForUpgrade(upgrade *cloudweavv1.Upgrade, repoInfoStr string) (*cloudweavv1.Upgrade, error) {
	...
	upgrade.Labels[upgradeStateLabel] = StatePreparingNodes
	upgrade.Status.RepoInfo = repoInfoStr
	cloudweavv1.NodesPrepared.CreateUnknownIfNotExists(upgrade)
	return h.upgradeClient.Update(upgrade)
}
...
func (h *upgradeHandler) saveReplicaReplenishmentToUpgradeAnnotation(upgrade *cloudweavv1.Upgrade) error {
	replicaReplenishmentWaitIntervalValue, err := h.getReplicaReplenishmentValue()
	if err != nil {
		return err
	}
...
func (h *upgradeHandler) loadReplicaReplenishmentFromUpgradeAnnotation(upgrade *cloudweavv1.Upgrade) error {
...
func (h *upgradeHandler) storeVMState(upgrade *cloudweavv1.Upgrade) error {
...
func (h *upgradeHandler) restoreVMState(upgrade *cloudweavv1.Upgrade) {
	logFields := logrus.Fields{
		"namespace": upgrade.Namespace,
		"name":      upgrade.Name,
	}
...
func (h *upgradeHandler) isSingleNodeRestoreVM(upgrade *cloudweavv1.Upgrade) (bool, error) {
	singleNode, err := h.isSingleNodeCluster()
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"namespace": upgrade.Namespace,
