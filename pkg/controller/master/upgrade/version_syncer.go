...
type versionSyncer struct {
	...
	versionClient   ctlcloudweavv1.VersionClient
	nodeClient      ctlcorev1.NodeClient
	namespaceClient ctlcorev1.NamespaceClient
}

func newVersionSyncer(ctx context.Context, namespace string, versions ctlcloudweavv1.VersionController, nodes ctlcorev1.NodeController, namespaces ctlcorev1.NamespaceController) *versionSyncer {
...
func (s *versionSyncer) getExtraInfo() (map[string]string, error) {
	...
	sysNamespace, err := s.namespaceClient.Get(util.CloudweavSystemNamespaceName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (s *versionSyncer) syncVersions(resp CheckUpgradeResponse, currentVersion string) error {
	if runtime.GOARCH == "arm64" {
		// wait until https://github.com/cloudweav/cloudweav/issues/6257 is resolved
		return nil
	}
...
func (s *versionSyncer) getNewVersion(v Version) (*cloudweavv1.Version, error) {
	...
	var newVersion cloudweavv1.Version
...
func canUpgrade(currentVersion string, newVersion *cloudweavv1.Version, responderVersion Version) bool {
	switch {
