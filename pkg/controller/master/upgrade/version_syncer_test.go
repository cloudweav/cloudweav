...
const (
	defaultNamespace = "cloudweav-system"
)

func TestGetUpgradableVersions(t *testing.T) {
	type input struct {
		newVersions    cloudweavv1.Version
		respVersion    Version
	...
	var testCases = []struct {
		...
		},
		{
			...
			given: input{
				...
				newVersions: cloudweavv1.Version{
					Spec: cloudweavv1.VersionSpec{
						ISOURL:      "http://server/cloudweav.iso",
						ISOChecksum: "",
					},
				},
			},
...
func Test_syncVersions(t *testing.T) {
	...
	vc := fakeclients.VersionClient(client.CloudweavhciV1beta1().Versions)
...
func generateResponse(rw http.ResponseWriter, v Version) {
	cloudweavVersion := cloudweavv1.Version{
		ObjectMeta: metav1.ObjectMeta{
			Name: v.Name,
		},
		Spec: cloudweavv1.VersionSpec{
	...
	err := json.NewEncoder(rw).Encode(&cloudweavVersion)
	if err != nil {
