...
import (
	...
	cloudweavutil "github.com/cloudweav/cloudweav/pkg/util"
)

// MonitorAddonRancherMonitoring will track status change of rancher-monitroing addon
func (h *Handler) MonitorAddonRancherMonitoring(_ string, aObj *cloudweavv1.Addon) (*cloudweavv1.Addon, error) {
	if aObj == nil || aObj.DeletionTimestamp != nil {
		return nil, nil
	}

	if aObj.Name != cloudweavutil.RancherMonitoringName || aObj.Namespace != cloudweavutil.CattleMonitoringSystemNamespaceName {
		return aObj, nil
	}

	// deployed successfully
	if aObj.Spec.Enabled && aObj.Status.Status == cloudweavv1.AddonDeployed {
		// patch pv ReclaimPolicy
		_, pvname, err := h.patchGrafanaPVReclaimPolicy(cloudweavutil.CattleMonitoringSystemNamespaceName, cloudweavutil.GrafanaPVCName)
	...
	if !aObj.Spec.Enabled && aObj.Status.Status == cloudweavv1.AddonDisabled {
...
func (h *Handler) annotateGrafanaPVName(pvname string, aObj *cloudweavv1.Addon) (*cloudweavv1.Addon, error) {
	if aObj == nil || aObj.DeletionTimestamp != nil {
		return nil, nil
	}
	...
	if aObj.Annotations != nil {
		...
		if p, _ := aObj.Annotations[cloudweavutil.AnnotationGrafanaPVName]; p == pvname {
			return aObj, nil
		}
	}
	...
	a.Annotations[cloudweavutil.AnnotationGrafanaPVName] = pvname
	return h.addon.Update(a)
}

func (h *Handler) patchGrafanaPVClaimRef(aObj *cloudweavv1.Addon) (*cloudweavv1.Addon, error) {
	if aObj == nil || aObj.DeletionTimestamp != nil {
		return nil, nil
	}
	...
	pvname := ""
	if aObj.Annotations != nil {
		if p, ok := aObj.Annotations[cloudweavutil.AnnotationGrafanaPVName]; ok {
			pvname = p
		}
	}
