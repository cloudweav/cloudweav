...
func (h *Handler) MonitorAddonPerStatus(_ string, aObj *cloudweavv1.Addon) (*cloudweavv1.Addon, error) {
...
// this is needed to ensure that addons cannot be upgrade directly from the apps with rancher mcm enabled on cloudweav
func (h *Handler) PatchApps(key string, app *catalogv1.App) (*catalogv1.App, error) {
	if app == nil || app.DeletionTimestamp != nil {
		return nil, nil
	}

	// check if cloudweav addon exists //
