...
const (
	...
	cronJobNamespace    = "cloudweav-system"
	cronJobBackoffLimit = 3
	cronJobCmd          = "sleep"
	cronJobArg          = "10"
)

var scheduleVMBackupKind = cloudweavv1.SchemeGroupVersion.WithKind(scheduleVMBackupKindName)

func cronJobName(svmbackup *cloudweavv1.ScheduleVMBackup) string {
	return fmt.Sprintf("%s-%s", svmbackupPrefix, svmbackup.UID)
}

func getCronJob(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) (*batchv1.CronJob, error) {
...
func vmBackupName(svmbackup *cloudweavv1.ScheduleVMBackup, timestamp string) string {
	return fmt.Sprintf("%s-%s-%s", svmbackupPrefix, svmbackup.UID, timestamp)
}

func getVMBackup(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup, timestamp string) (*cloudweavv1.VirtualMachineBackup, error) {
...
func currentVMBackups(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) (
	vmbackups []*cloudweavv1.VirtualMachineBackup, errVMBackups []*cloudweavv1.VirtualMachineBackup,
	lastVMBackup *cloudweavv1.VirtualMachineBackup, failure int, err error) {
	sets := labels.Set{
	...
	errVMBackups = []*cloudweavv1.VirtualMachineBackup{}
...
func createVMBackup(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup, timestamp string) (*cloudweavv1.VirtualMachineBackup, error) {
...
func cleanseVMBackup(h *svmbackupHandler, vmbackup *cloudweavv1.VirtualMachineBackup) error {
	propagation := metav1.DeletePropagationForeground
	return h.vmBackupClient.Delete(vmbackup.Namespace, vmbackup.Name,
		&metav1.DeleteOptions{PropagationPolicy: &propagation})
}

func clearVMBackups(h *svmbackupHandler, vmbackups []*cloudweavv1.VirtualMachineBackup,
...
func gcVMBackups(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
...
func convertVMBackupToInfo(vmbackup *cloudweavv1.VirtualMachineBackup) cloudweavv1.VMBackupInfo {
	var vmBackupInfo cloudweavv1.VMBackupInfo
	...
	vmBackupInfo.VolumeBackupInfo = make([]cloudweavv1.VolumeBackupInfo, len(status.VolumeBackups))
...
func reconcileVMBackupList(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
	...
	svmbackupCpy.Status.VMBackupInfo = make([]cloudweavv1.VMBackupInfo, len(vmbackups))
...
func updateVMBackups(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
	var errs error
...
func updateSuspendState(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup, suspend bool, reason, msg string) error {
	...
	if suspend {
		...
		cloudweavv1.BackupSuspend.True(svmbackupCpy)
		cloudweavv1.BackupSuspend.Reason(svmbackupCpy, reason)
		cloudweavv1.BackupSuspend.Message(svmbackupCpy, msg)
	} else {
		svmbackupCpy.Spec.Suspend = false
		svmbackupCpy.Status.Suspended = false
		cloudweavv1.BackupSuspend.False(svmbackupCpy)
		cloudweavv1.BackupSuspend.Reason(svmbackupCpy, "")
		cloudweavv1.BackupSuspend.Message(svmbackupCpy, "")
	}
...
func handleReachMaxFailure(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup, msg string) error {
	return updateSuspendState(h, svmbackup, true, reachMaxFailure, msg)
}

func handleLHBackupNotSynced(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup, msg string) error {
	return updateSuspendState(h, svmbackup, true, lhbackupNotSync, msg)
}

func handleSuspend(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
	return updateSuspendState(h, svmbackup, true, proactiveSuspend, proactiveSuspend)
}

func handleResume(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
...
func updateResumeOrSuspend(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
...
func updateCronExpression(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
	cronJob, err := getCronJob(h, svmbackup)
	if err != nil {
		return err
	}
...
func checkLHBackupUnexpectedProcessing(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup, vmbackup *cloudweavv1.VirtualMachineBackup) error {
...
func newVMBackups(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup, timestamp string) (*cloudweavv1.VirtualMachineBackup, error) {
