...
func deleteCronJob(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) error {
...
func createCronJob(h *svmbackupHandler, svmbackup *cloudweavv1.ScheduleVMBackup) (*batchv1.CronJob, error) {
	backoffLimit := int32(cronJobBackoffLimit)
	jobImage, err := utilCatalog.FetchAppChartImage(h.appCache, h.namespace,
		releaseAppCloudweavName, []string{"generalJob", "image"})
	if err != nil {
		return nil, fmt.Errorf("failed to get cloudweav image (%s): %v", jobImage.ImageName(), err)
	}
...
func (h *svmbackupHandler) OnChanged(_ string, svmbackup *cloudweavv1.ScheduleVMBackup) (*cloudweavv1.ScheduleVMBackup, error) {
...
func (h *svmbackupHandler) OnRemove(_ string, svmbackup *cloudweavv1.ScheduleVMBackup) (*cloudweavv1.ScheduleVMBackup, error) {
	if svmbackup == nil {
		return nil, nil
	}
