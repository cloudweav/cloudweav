...
var (
	...
	svmbackup = &cloudweavv1.ScheduleVMBackup{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "default",
			Name:      "svmbackup",
			UID:       "841f1346-ce15-4495-a5f6-fae76d11f92e",
		},
		Spec: cloudweavv1.ScheduleVMBackupSpec{
			Retain:     retain,
	...
	vmbackup1 = &cloudweavv1.VirtualMachineBackup{
		ObjectMeta: metav1.ObjectMeta{
		...
		Status: &cloudweavv1.VirtualMachineBackupStatus{
			ReadyToUse: pointer.Bool(true),
			Error:      nil,
		},
	}

	vmbackup1Info = &cloudweavv1.VMBackupInfo{
		Name:       vmbackup1.Name,
		ReadyToUse: pointer.Bool(true),
		Error:      nil,
	}

	vmbackup2 = &cloudweavv1.VirtualMachineBackup{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: svmbackup.Namespace,
	...
	vmbackup3 = &cloudweavv1.VirtualMachineBackup{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: svmbackup.Namespace,
	...
	vmbackupErr = &cloudweavv1.VirtualMachineBackup{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: svmbackup.Namespace,
...
func Test_GetVMBackup(t *testing.T) {
	...
	h := &svmbackupHandler{
		vmBackupCache: fakeclients.VMBackupCache(clientset.CloudweavhciV1beta1().VirtualMachineBackups),
	}
...
func Test_CreateVMBackup(t *testing.T) {
	...
	h := &svmbackupHandler{
		vmBackupClient: fakeclients.VMBackupClient(clientset.CloudweavhciV1beta1().VirtualMachineBackups),
	}
...
func Test_ReconcileVMBackupList(t *testing.T) {
	...
	h := &svmbackupHandler{
		vmBackupCache:   fakeclients.VMBackupCache(clientset.CloudweavhciV1beta1().VirtualMachineBackups),
		svmbackupClient: fakeclients.SVMBackupClient(clientset.CloudweavhciV1beta1().ScheduleVMBackups),
		svmbackupCache:  fakeclients.SVMBackupCache(clientset.CloudweavhciV1beta1().ScheduleVMBackups),
	}
