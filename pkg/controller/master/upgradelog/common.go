...
func upgradeLogReference(upgradeLog *cloudweavv1.UpgradeLog) metav1.OwnerReference {
...
func prepareOperator(upgradeLog *cloudweavv1.UpgradeLog) *mgmtv3.ManagedChart {
...
func prepareLogging(upgradeLog *cloudweavv1.UpgradeLog, images map[string]Image) *loggingv1.Logging {
...
func prepareClusterFlow(upgradeLog *cloudweavv1.UpgradeLog) *loggingv1.ClusterFlow {
	return &loggingv1.ClusterFlow{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: util.CloudweavSystemNamespaceName,
			OwnerReferences: []metav1.OwnerReference{
				upgradeLogReference(upgradeLog),
			},
		},
		Spec: loggingv1.ClusterFlowSpec{
			...
			Match: []loggingv1.ClusterMatch{
				...
				},
				{
					ClusterSelect: &loggingv1.ClusterSelect{
						Labels:     map[string]string{},
						Namespaces: []string{"cloudweav-system"},
					},
				},
				{
					ClusterSelect: &loggingv1.ClusterSelect{
...
func prepareClusterOutput(upgradeLog *cloudweavv1.UpgradeLog) *loggingv1.ClusterOutput {
...
func prepareLogDownloader(upgradeLog *cloudweavv1.UpgradeLog, imageVersion string) *appsv1.Deployment {
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
							...
							PreferredDuringSchedulingIgnoredDuringExecution: []corev1.WeightedPodAffinityTerm{
								{
									...
									PodAffinityTerm: corev1.PodAffinityTerm{
										...
										Namespaces: []string{
											util.CloudweavSystemNamespaceName,
										},
					...
					Containers: []corev1.Container{
						{
							Name:  "downloader",
							Image: fmt.Sprintf("%s:%s", util.CloudweavUpgradeImageRepository, imageVersion),
							Command: []string{
								"nginx", "-g", "daemon off;",
							},
...
func prepareLogDownloaderSvc(upgradeLog *cloudweavv1.UpgradeLog) *corev1.Service {
...
func GetUpgradeLogPvcName(upgradeLog *cloudweavv1.UpgradeLog) string {
...
func setOperatorDeployedCondition(upgradeLog *cloudweavv1.UpgradeLog, status corev1.ConditionStatus, reason, message string) {
	cloudweavv1.LoggingOperatorDeployed.SetStatus(upgradeLog, string(status))
	cloudweavv1.LoggingOperatorDeployed.Reason(upgradeLog, reason)
	cloudweavv1.LoggingOperatorDeployed.Message(upgradeLog, message)
}

func setInfraReadyCondition(upgradeLog *cloudweavv1.UpgradeLog, status corev1.ConditionStatus, reason, message string) {
	cloudweavv1.InfraReady.SetStatus(upgradeLog, string(status))
	cloudweavv1.InfraReady.Reason(upgradeLog, reason)
	cloudweavv1.InfraReady.Message(upgradeLog, message)
}

func setUpgradeLogReadyCondition(upgradeLog *cloudweavv1.UpgradeLog, status corev1.ConditionStatus, reason, message string) {
	cloudweavv1.UpgradeLogReady.SetStatus(upgradeLog, string(status))
	cloudweavv1.UpgradeLogReady.Reason(upgradeLog, reason)
	cloudweavv1.UpgradeLogReady.Message(upgradeLog, message)
}

func setUpgradeEndedCondition(upgradeLog *cloudweavv1.UpgradeLog, status corev1.ConditionStatus, reason, message string) {
	cloudweavv1.UpgradeEnded.SetStatus(upgradeLog, string(status))
	cloudweavv1.UpgradeEnded.Reason(upgradeLog, reason)
	cloudweavv1.UpgradeEnded.Message(upgradeLog, message)
}

func setDownloadReadyCondition(upgradeLog *cloudweavv1.UpgradeLog, status corev1.ConditionStatus, reason, message string) {
	cloudweavv1.DownloadReady.SetStatus(upgradeLog, string(status))
	cloudweavv1.DownloadReady.Reason(upgradeLog, reason)
	cloudweavv1.DownloadReady.Message(upgradeLog, message)
}

func setUpgradeLogArchiveReady(upgradeLog *cloudweavv1.UpgradeLog, archiveName string, ready bool, reason string) error {
...
func (p *upgradeBuilder) LogReadyCondition(status corev1.ConditionStatus, reason, message string) *upgradeBuilder {
	cloudweavv1.LogReady.SetStatus(p.upgrade, string(status))
	cloudweavv1.LogReady.Reason(p.upgrade, reason)
	cloudweavv1.LogReady.Message(p.upgrade, message)
	return p
}
...
type upgradeLogBuilder struct {
	upgradeLog *cloudweavv1.UpgradeLog
}
...
func (p *upgradeLogBuilder) Build() *cloudweavv1.UpgradeLog {
	return p.upgradeLog
}
...
func (p *addonBuilder) Build() *cloudweavv1.Addon {
	return p.addon
}
...
func SetUpgradeLogArchive(upgradeLog *cloudweavv1.UpgradeLog, archiveName string, archiveSize int64, generatedTime string) {
	if upgradeLog == nil {
		return
	}
	if upgradeLog.Status.Archives == nil {
		upgradeLog.Status.Archives = make(map[string]cloudweavv1.Archive, 1)
	}
	...
	upgradeLog.Status.Archives[archiveName] = cloudweavv1.Archive{
		Size:          archiveSize,
