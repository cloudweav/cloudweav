...
func newTestUpgradeLogBuilder() *upgradeLogBuilder {
	return newUpgradeLogBuilder(testUpgradeLogName).
		WithLabel(cloudweavUpgradeLabel, testUpgradeName).
...
func TestHandler_OnClusterFlowChange(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		var handler = &handler{
			...
			upgradeLogClient: fakeclients.UpgradeLogClient(clientset.CloudweavhciV1beta1().UpgradeLogs),
			upgradeLogCache:  fakeclients.UpgradeLogCache(clientset.CloudweavhciV1beta1().UpgradeLogs),
		}
		...
		if tc.expected.upgradeLog != nil {
			var err error
			actual.upgradeLog, err = handler.upgradeLogCache.Get(util.CloudweavSystemNamespaceName, testUpgradeLogName)
			assert.Nil(t, err)
...
func TestHandler_OnUpgradeLogChange(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		var handler = &handler{
			namespace:           util.CloudweavSystemNamespaceName,
			addonCache:          fakeclients.AddonCache(clientset.CloudweavhciV1beta1().Addons),
		...
		if tc.expected.clusterFlow != nil {
			...
			var err error
			actual.clusterFlow, err = handler.clusterFlowClient.Get(util.CloudweavSystemNamespaceName, name.SafeConcatName(testUpgradeLogName, util.UpgradeLogFlowComponent), metav1.GetOptions{})
			assert.Nil(t, err)
		...
		if tc.expected.clusterOutput != nil {
			...
			var err error
			actual.clusterOutput, err = handler.clusterOutputClient.Get(util.CloudweavSystemNamespaceName, name.SafeConcatName(testUpgradeLogName, util.UpgradeLogOutputComponent), metav1.GetOptions{})
			assert.Nil(t, err)
		...
		if tc.expected.deployment != nil {
			var err error
			actual.deployment, err = handler.deploymentClient.Get(util.CloudweavSystemNamespaceName, name.SafeConcatName(testUpgradeLogName, util.UpgradeLogDownloaderComponent), metav1.GetOptions{})
			assert.Nil(t, err)
		}
		...
		if tc.expected.pvc != nil {
			var err error
			actual.pvc, err = handler.pvcClient.Get(util.CloudweavSystemNamespaceName, name.SafeConcatName(testUpgradeLogName, util.UpgradeLogArchiveComponent), metav1.GetOptions{})
			assert.Nil(t, err)
		...
		if tc.expected.service != nil {
			var err error
			actual.service, err = handler.serviceClient.Get(util.CloudweavSystemNamespaceName, testUpgradeLogName, metav1.GetOptions{})
			assert.Nil(t, err)
		...
		if tc.expected.upgrade != nil {
			var err error
			actual.upgrade, err = handler.upgradeCache.Get(util.CloudweavSystemNamespaceName, testUpgradeName)
			assert.Nil(t, err)
			emptyConditionsTime(tc.expected.upgrade.Status.Conditions)
			emptyConditionsTime(actual.upgrade.Status.Conditions)
			assert.Equal(t, tc.expected.upgrade, actual.upgrade, "case %q", tc.name)
...
func emptyConditionsTime(conditions []cloudweavv1.Condition) {
