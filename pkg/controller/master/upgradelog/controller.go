...
import (
	...
	ctlloggingv1 "github.com/cloudweav/cloudweav/pkg/generated/controllers/logging.banzaicloud.io/v1beta1"
	"github.com/cloudweav/cloudweav/pkg/util"
)

const (
	// Annotations for the sub-components of logging infrastructure
	upgradeLogFluentBitAnnotation     = "cloudweavhci.io/fluentBit"
	upgradeLogFluentdAnnotation       = "cloudweavhci.io/fluentd"
	upgradeLogFluentBitReady          = "FluentBitReady"
	upgradeLogFluentdReady            = "FluentdReady"
	upgradeLogClusterFlowAnnotation   = "cloudweavhci.io/clusterFlow"
	upgradeLogClusterOutputAnnotation = "cloudweavhci.io/clusterOutput"
	...
	upgradeLogStateAnnotation = "cloudweavhci.io/upgradeLogState"
...
type handler struct {
	ctx                 context.Context
	namespace           string
	addonCache          ctlcloudweavv1.AddonCache
...
func (h *handler) OnUpgradeLogChange(_ string, upgradeLog *cloudweavv1.UpgradeLog) (*cloudweavv1.UpgradeLog, error) {
	...
	if cloudweavv1.LoggingOperatorDeployed.IsTrue(upgradeLog) && cloudweavv1.InfraReady.GetStatus(upgradeLog) == "" {
		...
		cloudweavv1.InfraReady.CreateUnknownIfNotExists(toUpdate)

		return h.upgradeLogClient.Update(toUpdate)
	} else if cloudweavv1.LoggingOperatorDeployed.IsTrue(upgradeLog) && cloudweavv1.InfraReady.IsUnknown(upgradeLog) {
	...
	if cloudweavv1.InfraReady.IsTrue(upgradeLog) && cloudweavv1.UpgradeLogReady.IsUnknown(upgradeLog) {
	...
	if cloudweavv1.UpgradeLogReady.IsTrue(upgradeLog) && cloudweavv1.UpgradeEnded.GetStatus(upgradeLog) == "" {
		...
		upgrade, err := h.upgradeCache.Get(util.CloudweavSystemNamespaceName, upgradeName)
		if err != nil {
		...
		cloudweavv1.LogReady.SetStatus(upgradeToUpdate, string(corev1.ConditionTrue))
		cloudweavv1.LogReady.Reason(upgradeToUpdate, "")
		cloudweavv1.LogReady.Message(upgradeToUpdate, "")
		...
		cloudweavv1.UpgradeEnded.CreateUnknownIfNotExists(toUpdate)

		return h.upgradeLogClient.Update(toUpdate)
	}

	// Spin up the log downloader to serve the log downloading requests
	if cloudweavv1.UpgradeEnded.IsUnknown(upgradeLog) && cloudweavv1.DownloadReady.GetStatus(upgradeLog) == "" {
		...
		toUpdate := upgradeLog.DeepCopy()
		cloudweavv1.DownloadReady.CreateUnknownIfNotExists(toUpdate)

		return h.upgradeLogClient.Update(toUpdate)
	}

	// Tear down the loggin infrastructure but keep the log downloader and the archive volume
	if cloudweavv1.UpgradeEnded.IsTrue(upgradeLog) {
...
func (h *handler) OnUpgradeLogRemove(_ string, upgradeLog *cloudweavv1.UpgradeLog) (*cloudweavv1.UpgradeLog, error) {
	if upgradeLog == nil {
		return nil, nil
	}
...
func (h *handler) OnClusterFlowChange(_ string, clusterFlow *loggingv1.ClusterFlow) (*loggingv1.ClusterFlow, error) {
	if clusterFlow == nil || clusterFlow.DeletionTimestamp != nil || clusterFlow.Labels == nil || clusterFlow.Namespace != util.CloudweavSystemNamespaceName {
	...
	upgradeLog, err := h.upgradeLogCache.Get(util.CloudweavSystemNamespaceName, upgradeLogName)
	if err != nil {
...
func (h *handler) OnClusterOutputChange(_ string, clusterOutput *loggingv1.ClusterOutput) (*loggingv1.ClusterOutput, error) {
	if clusterOutput == nil || clusterOutput.DeletionTimestamp != nil || clusterOutput.Labels == nil || clusterOutput.Namespace != util.CloudweavSystemNamespaceName {
...
func (h *handler) OnDaemonSetChange(_ string, daemonSet *appsv1.DaemonSet) (*appsv1.DaemonSet, error) {
	if daemonSet == nil || daemonSet.DeletionTimestamp != nil || daemonSet.Labels == nil || daemonSet.Namespace != util.CloudweavSystemNamespaceName {
...
func (h *handler) OnDeploymentChange(_ string, deployment *appsv1.Deployment) (*appsv1.Deployment, error) {
	if deployment == nil || deployment.DeletionTimestamp != nil || deployment.Labels == nil || deployment.Namespace != util.CloudweavSystemNamespaceName {
...
func (h *handler) OnStatefulSetChange(_ string, statefulSet *appsv1.StatefulSet) (*appsv1.StatefulSet, error) {
	if statefulSet == nil || statefulSet.DeletionTimestamp != nil || statefulSet.Labels == nil || statefulSet.Namespace != util.CloudweavSystemNamespaceName {
...
func (h *handler) OnPvcChange(_ string, pvc *corev1.PersistentVolumeClaim) (*corev1.PersistentVolumeClaim, error) {
	if pvc == nil || pvc.DeletionTimestamp != nil || pvc.Namespace != util.CloudweavSystemNamespaceName {
		return pvc, nil
	}
...
func (h *handler) OnUpgradeChange(_ string, upgrade *cloudweavv1.Upgrade) (*cloudweavv1.Upgrade, error) {
	if upgrade == nil || upgrade.DeletionTimestamp != nil || upgrade.Labels == nil || upgrade.Namespace != util.CloudweavSystemNamespaceName {
		return upgrade, nil
	}
	...
	if upgrade.Labels[util.LabelUpgradeReadMessage] == "true" {
		...
		if err != nil {
			if apierrors.IsNotFound(err) {
				logrus.Debugf("The corresponding UpgradeLog %s/%s is not found, skip purging", util.CloudweavSystemNamespaceName, upgradeLogName)
				return upgrade, nil
			}
			return nil, err
		}
...
func (h *handler) stopCollect(upgradeLog *cloudweavv1.UpgradeLog) error {
	logrus.Info("Tearing down the logging infrastructure for upgrade procedure")

	var err error
	err = h.clusterFlowClient.Delete(util.CloudweavSystemNamespaceName, name.SafeConcatName(upgradeLog.Name, util.UpgradeLogFlowComponent), &metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		return err
	}
	err = h.clusterOutputClient.Delete(util.CloudweavSystemNamespaceName, name.SafeConcatName(upgradeLog.Name, util.UpgradeLogOutputComponent), &metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		return err
	}
...
func (h *handler) cleanup(upgradeLog *cloudweavv1.UpgradeLog) error {
