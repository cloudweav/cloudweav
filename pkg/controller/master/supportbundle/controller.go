...
import (
	...
	settingctl "github.com/cloudweav/cloudweav/pkg/controller/master/setting"
	...
	supportBundleUtil "github.com/cloudweav/cloudweav/pkg/util/supportbundle"
)
...
func (h *Handler) OnSupportBundleChanged(_ string, sb *cloudweavv1.SupportBundle) (*cloudweavv1.SupportBundle, error) {
...
func (h *Handler) checkExistTime(sb *cloudweavv1.SupportBundle) (*cloudweavv1.SupportBundle, error) {
	t, err := time.Parse(time.RFC3339, cloudweavv1.SupportBundleInitialized.GetLastUpdated(sb))
	if err != nil {
		logrus.Debugf("[%s] fail to parse %s", sb.Name, cloudweavv1.SupportBundleInitialized.GetLastUpdated(sb))
		return sb, err
	}
...
func (h *Handler) updateSupportBundleImageSetting() error {
	cloudweavManagedChart, err := h.managedChartCache.Get(settingctl.ManagedChartNamespace, settingctl.CloudweavManagedChartName)
	if err != nil {
		return err
	}

	app, err := h.appCache.Get(cloudweavManagedChart.Spec.DefaultNamespace, cloudweavManagedChart.Spec.ReleaseName)
	if err != nil {
		return err
	}
...
func (h *Handler) checkManagerStatus(sb *cloudweavv1.SupportBundle) (*cloudweavv1.SupportBundle, error) {
	var timeout int
	var err error
...
func (h *Handler) setError(sb *cloudweavv1.SupportBundle, reason string) (*cloudweavv1.SupportBundle, error) {
	logrus.Errorf("[%s] set state to error: %s", sb.Name, reason)
	toUpdate := sb.DeepCopy()
	cloudweavv1.SupportBundleInitialized.False(toUpdate)
	cloudweavv1.SupportBundleInitialized.Message(toUpdate, reason)
	toUpdate.Status.State = types.StateError
	return h.supportBundles.Update(toUpdate)
}

func (h *Handler) setState(sb *cloudweavv1.SupportBundle, state string) (*cloudweavv1.SupportBundle, error) {
...
func (h *Handler) setReady(sb *cloudweavv1.SupportBundle, filename string, filesize int64) (*cloudweavv1.SupportBundle, error) {
	logrus.Debugf("[%s] set state to %s", sb.Name, types.StateReady)
	toUpdate := sb.DeepCopy()
	cloudweavv1.SupportBundleInitialized.True(toUpdate)
...
func (h *Handler) setProgress(sb *cloudweavv1.SupportBundle, progress int) (*cloudweavv1.SupportBundle, error) {
