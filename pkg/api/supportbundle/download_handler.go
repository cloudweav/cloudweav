...
import (
	...
	ctlsb "github.com/cloudweav/cloudweav/pkg/controller/master/supportbundle"
...
func NewDownloadHandler(scaled *config.Scaled, namespace string) *DownloadHandler {
	return &DownloadHandler{
		context:            scaled.Ctx,
		namespace:          namespace,
		supportBundles:     scaled.CloudweavFactory.Cloudweavhci().V1beta1().SupportBundle(),
		supportBundleCache: scaled.CloudweavFactory.Cloudweavhci().V1beta1().SupportBundle().Cache(),
...
func (h *DownloadHandler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if err == nil {
			var errResp cloudweavv1.ErrorResponse
