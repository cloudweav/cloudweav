...
import (
	...
	virtv1 "github.com/cloudweav/cloudweav/pkg/generated/clientset/versioned/typed/kubevirt.io/v1"
)
...
func RegisterSchema(scaled *config.Scaled, server *server.Server, options config.Options) error {
	...
	backups := scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineBackup()
	restores := scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineRestore()
	settings := scaled.CloudweavFactory.Cloudweavhci().V1beta1().Setting()
	...
	vmt := scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineTemplate()
	vmtv := scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineTemplateVersion()
	vmImages := scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineImage()
	storageClasses := scaled.StorageFactory.Storage().V1().StorageClass()
	nads := scaled.CniFactory.K8s().V1().NetworkAttachmentDefinition()
	resourceQuotas := scaled.Management.CloudweavFactory.Cloudweavhci().V1beta1().ResourceQuota()
