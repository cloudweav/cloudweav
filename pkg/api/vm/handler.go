...
import (
	...
	apiutil "github.com/cloudweav/cloudweav/pkg/api/util"
	volumeapi "github.com/cloudweav/cloudweav/pkg/api/volume"
	cloudweavv1 "github.com/cloudweav/cloudweav/pkg/apis/cloudweavhci.io/v1beta1"
	"github.com/cloudweav/cloudweav/pkg/builder"
	nodecontroller "github.com/cloudweav/cloudweav/pkg/controller/master/node"
	...
	"github.com/cloudweav/cloudweav/pkg/util/drainhelper"
)

const (
	vmResource    = "virtualmachines"
	vmiResource   = "virtualmachineinstances"
	sshAnnotation = "cloudweavhci.io/sshNames"
)
...
type vmActionHandler struct {
	...
	vmTemplateClient          ctlcloudweavv1.VirtualMachineTemplateClient
	vmTemplateVersionClient   ctlcloudweavv1.VirtualMachineTemplateVersionClient
	vmimCache                 ctlkubevirtv1.VirtualMachineInstanceMigrationCache
	backups                   ctlcloudweavv1.VirtualMachineBackupClient
	...
	vmImages                  ctlcloudweavv1.VirtualMachineImageClient
	vmImageCache              ctlcloudweavv1.VirtualMachineImageCache
	storageClassCache         ctlstoragev1.StorageClassCache
	resourceQuotaClient       ctlcloudweavv1.ResourceQuotaClient
...
func (h *vmActionHandler) createVMBackup(vmName, vmNamespace string, input BackupInput) error {
	...
	backup := &cloudweavv1.VirtualMachineBackup{
		...
		Spec: cloudweavv1.VirtualMachineBackupSpec{
			Source: corev1.TypedLocalObjectReference{
				APIGroup: &apiGroup,
				Kind:     kubevirtv1.VirtualMachineGroupVersionKind.Kind,
				Name:     vmName,
			},
			Type: cloudweavv1.Backup,
		},
	}
...
func (h *vmActionHandler) restoreBackup(vmName, vmNamespace string, input RestoreInput) error {
	...
	apiGroup := kubevirtv1.SchemeGroupVersion.Group
	restore := &cloudweavv1.VirtualMachineRestore{
		ObjectMeta: metav1.ObjectMeta{
			Name:      input.Name,
			Namespace: vmNamespace,
		},
		Spec: cloudweavv1.VirtualMachineRestoreSpec{
...
func (h *vmActionHandler) checkBackupTargetConfigured() error {
	...
	if err == nil && cloudweavv1.SettingConfigured.IsTrue(targetSetting) {
...
func (h *vmActionHandler) createTemplate(namespace, name string, input CreateTemplateInput) error {
	...
	vmt, err := h.vmTemplateClient.Create(
		&cloudweavv1.VirtualMachineTemplate{
			ObjectMeta: metav1.ObjectMeta{
				Name:      input.Name,
				Namespace: namespace,
			},
	...
	vmtv, err := h.vmTemplateVersionClient.Create(
		&cloudweavv1.VirtualMachineTemplateVersion{
			ObjectMeta: metav1.ObjectMeta{
				Name:      vmtvName,
				Namespace: namespace,
			},
...
func (h *vmActionHandler) createSecrets(templateVersion *cloudweavv1.VirtualMachineTemplateVersion, vm *kubevirtv1.VirtualMachine) error {
...
func (h *vmActionHandler) copySecret(sourceName, targetName string, templateVersion *cloudweavv1.VirtualMachineTemplateVersion) error {
...
func (h *vmActionHandler) createVMImages(templateVersion *cloudweavv1.VirtualMachineTemplateVersion, vm *kubevirtv1.VirtualMachine, pvcStorageClassMap map[string]string) error {
	for index, volume := range vm.Spec.Template.Spec.Volumes {
		if volume.PersistentVolumeClaim == nil {
			continue
		}
		vmImageName := getTemplateVersionVMImageName(templateVersion.Name, index)
		vmImage := &cloudweavv1.VirtualMachineImage{
			ObjectMeta: metav1.ObjectMeta{
				Name:      vmImageName,
			...
			Spec: cloudweavv1.VirtualMachineImageSpec{
				...
				SourceType:   cloudweavv1.VirtualMachineImageSourceTypeExportVolume,
...
func (h *vmActionHandler) checkAttachable(pvc *corev1.PersistentVolumeClaim) error {
	// Even Cloudweav specify default StorageClass,
...
func (h *vmActionHandler) sanitizeVirtualMachineForTemplateVersion(templateVersionName string, vm *kubevirtv1.VirtualMachine, withData bool) (cloudweavv1.VirtualMachineSourceSpec, error) {
	var err error
	...
	if withData {
		...
		if err != nil {
			return cloudweavv1.VirtualMachineSourceSpec{}, err
		}
	}

	return cloudweavv1.VirtualMachineSourceSpec{
...
func (h *vmActionHandler) updateResourceQuota(namespace, name string, input UpdateResourceQuotaInput) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			resourceQuota = &cloudweavv1.ResourceQuota{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: namespace,
					Name:      util.DefaultResourceQuotaName,
				},
				Spec: cloudweavv1.ResourceQuotaSpec{
					SnapshotLimit: cloudweavv1.SnapshotLimit{
