...
func RegisterSchema(scaled *config.Scaled, server *server.Server, _ config.Options) error {
	...
	actionHandler := &ActionHandler{
		...
		pvs:         scaled.CloudweavCoreFactory.Core().V1().PersistentVolume(),
		pvCache:     scaled.CloudweavCoreFactory.Core().V1().PersistentVolume().Cache(),
	...
	handler := cloudweavServer.NewHandler(actionHandler)
