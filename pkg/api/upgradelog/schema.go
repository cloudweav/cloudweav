...
func RegisterSchema(scaled *config.Scaled, server *server.Server, _ config.Options) error {
	upgradeLogHandler := Handler{
		...
		upgradeCache:     scaled.CloudweavFactory.Cloudweavhci().V1beta1().Upgrade().Cache(),
		upgradeLogCache:  scaled.CloudweavFactory.Cloudweavhci().V1beta1().UpgradeLog().Cache(),
		upgradeLogClient: scaled.CloudweavFactory.Cloudweavhci().V1beta1().UpgradeLog(),
	}

	t := schema.Template{
		ID: "cloudweavhci.io.upgradelog",
