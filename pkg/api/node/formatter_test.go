...
var (
	testNode = &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				"network.cloudweavhci.io/secondary": "true",
			},
		},
	}
	...
	seederAddon = &cloudweavv1beta1.Addon{
		ObjectMeta: metav1.ObjectMeta{
			Name:      seederAddonName,
			Namespace: defaultAddonNamespace,
		},
		Spec: cloudweavv1beta1.AddonSpec{
			Enabled: true,
		},
	}
						...
						NodeSelectorTerms: []corev1.NodeSelectorTerm{
							{
								MatchExpressions: []corev1.NodeSelectorRequirement{
									{
										Key:      "network.cloudweavhci.io/secondary",
										Operator: corev1.NodeSelectorOpIn,
										Values:   []string{"true"},
									},
								},
...
func Test_powerActionNotPossible(t *testing.T) {
	...
	err := cloudweavv1beta1.AddToScheme(scheme.Scheme)
	...
	h := ActionHandler{
		nodeCache:     fakeclients.NodeCache(k8sclientset.CoreV1().Nodes),
		nodeClient:    fakeclients.NodeClient(k8sclientset.CoreV1().Nodes),
		addonCache:    fakeclients.AddonCache(client.CloudweavhciV1beta1().Addons),
