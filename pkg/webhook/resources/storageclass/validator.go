...
const (
	errorMessageReservedStorageClass = "storage class %s is reserved by Cloudweav and can't be deleted"
)
...
func NewValidator(storageClassCache ctlstoragev1.StorageClassCache, secretCache ctlcorev1.SecretCache, vmimagesCache ctlcloudweavv1.VirtualMachineImageCache) types.Validator {
...
type storageClassValidator struct {
	...
	vmimagesCache     ctlcloudweavv1.VirtualMachineImageCache
}
...
// Cloudweav rejects setting dataLocality as strict-local, because it makes volume non migrateable,
...
// The legacy `cloudweav-longhorn` SC is created from helm chart and monitored by the managedchart, also used by rancher-monitoring.
...
func (v *storageClassValidator) validateReservedStorageClass(sc *storagev1.StorageClass) error {
	...
	// Legacy cloudweav-longhorn may have no AnnotationIsReservedStorageClass, when it is deployed via helm (managedchart is on top of helm) then deny.
	if sc.Name == util.StorageClassCloudweavLonghorn {
		if sc.Annotations[util.HelmReleaseNamespaceAnnotation] == util.CloudweavSystemNamespaceName && sc.Annotations[util.HelmReleaseNameAnnotation] == util.CloudweavChartReleaseName {
			return werror.NewInvalidError(fmt.Sprintf(errorMessageReservedStorageClass, sc.Name), "")
		}
	}
