...
import (
	...
	cloudweavFake "github.com/cloudweav/cloudweav/pkg/generated/clientset/versioned/fake"
...
func Test_storageClassValidator_validateEncryption(t *testing.T) {
	...
	cloudweavClientSet := cloudweavFake.NewSimpleClientset()
	fakeVMIMageCache := fakeclients.VirtualMachineImageCache(cloudweavClientSet.CloudweavhciV1beta1().VirtualMachineImages)
...
func Test_storageClassValidator_Delete(t *testing.T) {
	tests := []struct {
		...
		{
			name: "storage class cloudweav-longhorn with AnnotationIsReservedStorageClass false can be deleted too",
			storageClass: &storagev1.StorageClass{
				ObjectMeta: metav1.ObjectMeta{
					Name: util.StorageClassCloudweavLonghorn,
					Annotations: map[string]string{
						util.AnnotationIsReservedStorageClass: "false",
					},
				},
		...
		{
			name: "storage class cloudweav-longhorn without AnnotationIsReservedStorageClass can't be deleted",
			storageClass: &storagev1.StorageClass{
				ObjectMeta: metav1.ObjectMeta{
					Name: util.StorageClassCloudweavLonghorn,
					Annotations: map[string]string{
						util.HelmReleaseNameAnnotation:      util.CloudweavChartReleaseName,
						util.HelmReleaseNamespaceAnnotation: util.CloudweavSystemNamespaceName,
					},
				},
			},
			expectError: true,
	...
	cloudweavClientSet := cloudweavFake.NewSimpleClientset(&v1beta1.VirtualMachineImage{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-vmimage",
			Namespace: "default",
			Annotations: map[string]string{
