...
func TestPatchResourceOvercommit(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			mutator := NewMutator(fakeclients.CloudweavSettingCache(clientset.CloudweavhciV1beta1().Settings),
			...
			vm := &kubevirtv1.VirtualMachine{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"cloudweavhci.io/reservedMemory": "384Mi",
					},
				},
				Spec: kubevirtv1.VirtualMachineSpec{
					Template: &kubevirtv1.VirtualMachineInstanceTemplateSpec{
	...
	for _, tc := range tests2 {
		t.Run(tc.name, func(t *testing.T) {
			...
			vm := &kubevirtv1.VirtualMachine{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"cloudweavhci.io/reservedMemory": "1Gi",
					},
				},
				Spec: kubevirtv1.VirtualMachineSpec{
					Template: &kubevirtv1.VirtualMachineInstanceTemplateSpec{
								...
								NodeSelectorTerms: []v1.NodeSelectorTerm{
									{
										MatchExpressions: []v1.NodeSelectorRequirement{
											{
												Key:      "network.cloudweavhci.io/to-delete-after-mutating",
								...
								NodeSelectorTerms: []v1.NodeSelectorTerm{
									{
										MatchExpressions: []v1.NodeSelectorRequirement{
											{
												Key:      "network.cloudweavhci.io/bbbb",
												Operator: v1.NodeSelectorOpIn,
												Values:   []string{"true"},
											},
										},
	...
	net1 := &cniv1.NetworkAttachmentDefinition{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"network.cloudweavhci.io/clusternetwork": "mgmt",
				"network.cloudweavhci.io/type":           "L2VlanNetwork",
				"network.cloudweavhci.io/vlan-id":        "178",
			},
		},
