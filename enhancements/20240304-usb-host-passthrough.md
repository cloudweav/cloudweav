...
Users can pass through USB devices which are attached on the nodes to virtual machine on Cloudweav instead of passing whole USB controller. It allows nodes and virtual machine be able to use USB devices in same USB controller, except the same one.
...
https://github.com/cloudweav/cloudweav/issues/1710
...
Overall, we will implement this feature in the cloudweav/pcidevices. So, the architecture will be similar. 
...
Regarding the resource and API path in cloudweav, it should be `usbdevices.devices.cloudweavhci.io` and `usbdeviceclaims.devices.cloudweavhci.io`.
