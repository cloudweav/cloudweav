...
https://github.com/cloudweav/cloudweav/issues/2274

## Motivation

### Goals
We need a way to allow users to browse their VM workloads running on vmware, openstack or cloudweav clusters and migrate them to current cloudweav cluster.

Currently there is no easy way to migrate VM's from other providers to cloudweav.
...
#### Migrate a VM from Vsphere/Openstack/Cloudweav cluster
* A user should be able to define a source vsphere/openstack or cloudweav cluster
* The user should be able to browse the cluster to identify a VM, and configure its network mapping to cloudweav VLAN definitions.
* After a period of time the user should be able to access the original VM running on Cloudweav.
...
```yaml:
apiVersion: source.cloudweavhci.io/v1
kind: Vsphere
metadata:
    name: cluster-name
spec:
...
```yaml:
apiVersion: importjob.cloudweavhci.io/v1
kind: VirtualMachine
metadata:
    name: VM-Name
    namespace: Namespace
...
No api changes in core cloudweav. The additional CRD's will be introduced in the VM Migration controller.
The integration with cloudweav will be managed as ManagedChart.

## Design
The controller will introduce two new Groups:

* source.cloudweavhci.io
* importjob.cloudweavhci.io

source.cloudweavhci.io will support the following kinds based on cluster providers:
* vsphere
* openstack
* cloudweav (eventually)

importjob.cloudweavhci.io will support the kind `VirtualMachine` and will refer to one of the source.cloudweav.hci kinds to identify the VM on source cluster and perform migration.
