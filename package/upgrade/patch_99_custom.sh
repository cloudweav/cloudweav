...
patch_92_cloudweav_kube_audit_policy() {
  TARGET_FILE=$1

  # note: the script may be called multi times as the JOB retries, skip already patched
  TARGET_INDEX=$(yq e '.stages.initramfs[0].files[].path' $TARGET_FILE | grep "92-cloudweav-kube-audit-policy.yaml" -n | sed -e "s/:.*//") || true
  ...
  # add new file 92-cloudweav-kube-audit-policy.yaml
  yq e '.stages.initramfs[0].files += [{"path": "/etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml", "permissions": 384, "owner": 0, "group": 0, "encoding": "", "ownerstring": "" }]' $TARGET_FILE -i
  ...
  if [ -n "$TARGET_INDEX" ]; then
    ...
    echo "can not find newly added 92-cloudweav-kube-audit-policy.yaml in $TARGET_FILE, CHECK"
    return 0
  fi

  LINE_NO=$(grep "path: /etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml" $TARGET_FILE -n | sed -e "s/:.*//") || true

  if [ -n "$LINE_NO" ]; then
    LINE_NO=$((LINE_NO+4)) # from 'path' to 'content'
    echo "the target line numbert is $LINE_NO"
  else
     echo "can not find the anchor keyword 92-cloudweav-kube-audit-policy.yaml, break"
     return 0
  fi
...
#        - path: /etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml
#          permissions: 384
#          owner: 0
#          group: 0
#          content: |
...
# add audit-policy-file param to 90-cloudweav-server.yaml
patch_90_cloudweav_server() {
  TARGET_FILE=$1

  ALREADY_SET=$(grep "audit-policy-file: /etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml" $TARGET_FILE) || true
  ...
  TARGET_INDEX=$(yq e '.stages.initramfs[0].files[].path' $TARGET_FILE | grep "90-cloudweav-server.yaml" -n | sed -e "s/:.*//") || true
  ...
  if [ -n "$TARGET_INDEX" ]; then
    ...
    echo "can not find  90-cloudweav-server.yaml in $TARGET_FILE, CHECK"
    return 0
  fi
  ...
  #indent 12
  AUDIT_POLICY_FILE_PARAM="\ \ \ \ \ \ \ \ \ \ \ \ audit-policy-file: /etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml"
...
patch_99_custom () {
  ...
  patch_92_cloudweav_kube_audit_policy $TEMP_FILE
  ...
  if test "$PATCH_SERVER_IN_CUSTOM" -eq 1; then
    patch_90_cloudweav_server $TEMP_FILE
  ...
  if test "$PATCH_SERVER_IN_CUSTOM" -eq 1; then
    yq e '.stages.initramfs[0].files[] | select(.path== "/etc/rancher/rke2/config.yaml.d/90-cloudweav-server.yaml")' $SRC_FILE
  fi

  yq e '.stages.initramfs[0].files[] | select(.path== "/etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml")' $SRC_FILE
}
