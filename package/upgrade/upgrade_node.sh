...
#!/bin/bash -ex
CLOUDWEAV_UPGRADE_POD_NAME=$CLOUDWEAV_UPGRADE_POD_NAME

EOF

  cat >> $HOST_DIR/tmp/upgrade-reboot.sh << 'EOF'
source /etc/bash.bashrc.local
pod_id=$(crictl pods --name $CLOUDWEAV_UPGRADE_POD_NAME --namespace cloudweav-system -o json | jq -er '.items[0].id')
...
if [ "$container_state" = "CONTAINER_EXITED" ]; then
  ...
  if [ "$container_exit_code" = "0" ]; then
    ...
    # workaround for https://github.com/cloudweav/cloudweav/issues/2865
...
get_running_vm_count()
{
  ...
  kubectl get vmi -A -l kubevirt.io/nodeName=$CLOUDWEAV_UPGRADE_NODE_NAME -o yaml > "$vmioutput" || EXIT_CODE=$?
...
wait_vms_out_or_shutdown()
{
  ...
  echo "all VMs on node $CLOUDWEAV_UPGRADE_NODE_NAME have been live-migrated or shutdown"
}
...
shutdown_repo_vm()
{
  ...
  repo_vm_name="upgrade-repo-$CLOUDWEAV_UPGRADE_NAME"
  repo_node=$(kubectl get vmi -n cloudweav-system $repo_vm_name -o yaml | yq -e e '.status.nodeName' -)
  if [ "$repo_node" = "$CLOUDWEAV_UPGRADE_NODE_NAME" ]; then
    echo "Stop upgrade repo VM: $repo_vm_name"
    virtctl stop $repo_vm_name -n cloudweav-system
  fi
}

shutdown_vms_on_node()
{
  kubectl get vmi -A -l kubevirt.io/nodeName=$CLOUDWEAV_UPGRADE_NODE_NAME -o json |
...
wait_longhorn_engines() {
  node_count=$(kubectl get nodes --selector=cloudweavhci.io/managed=true -o json | jq -r '.items | length')
...
patch_logging_event_audit()
{
  ...
  # get UPGRADE_PREVIOUS_VERSION and NODE_CURRENT_CLOUDWEAV_VERSION
  detect_upgrade
  detect_node_current_cloudweav_version
  echo "The UPGRADE_PREVIOUS_VERSION is $UPGRADE_PREVIOUS_VERSION, NODE_CURRENT_CLOUDWEAV_VERSION is $NODE_CURRENT_CLOUDWEAV_VERSION, will check Logging Event Audit upgrade node option"

  if test "$UPGRADE_PREVIOUS_VERSION" = "v1.0.3" || test "$NODE_CURRENT_CLOUDWEAV_VERSION" = "v1.0.3"; then
    echo "Patch kube-audit policy file"
    # this file should be there in each NODE
    # keep syncing with cloudweav/cloudweav-installer/pkg/config/templates/rke2-92-cloudweav-kube-audit-policy.yaml

    ls /host/etc/rancher/rke2/config.yaml.d/ -alt

    KUBE_AUDIT_POLICY_FILE_IN_CONTAINER=/host/etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml
    KUBE_AUDIT_POLICY_FILE_IN_HOST=/etc/rancher/rke2/config.yaml.d/92-cloudweav-kube-audit-policy.yaml
...
EOF
    ...
    # it means the NODE role is rke2-server when 90-cloudweav-server.yaml exists, patch it
    RKE2_SERVER_CONFIG_FILE_IN_CONTAINER=/host/etc/rancher/rke2/config.yaml.d/90-cloudweav-server.yaml
...
command_pre_drain() {
  ...
  upgrade-helper vm-live-migrate-detector "$CLOUDWEAV_UPGRADE_NODE_NAME" --shutdown

  # Live migrate VMs
  kubectl taint node $CLOUDWEAV_UPGRADE_NODE_NAME --overwrite kubevirt.io/drain=draining:NoSchedule
...
get_node_rke2_version() {
  kubectl get node $CLOUDWEAV_UPGRADE_NODE_NAME -o yaml | yq -e e '.status.nodeInfo.kubeletVersion' -
}
...
# Disable snapshot-controller related charts because we manage them in Cloudweav.
...
convert_nodenetwork_to_vlanconfig() {
  ...
  detect_node_current_cloudweav_version
  echo "The UPGRADE_PREVIOUS_VERSION is $UPGRADE_PREVIOUS_VERSION, NODE_CURRENT_CLOUDWEAV_VERSION is $NODE_CURRENT_CLOUDWEAV_VERSION, will check nodenetwork upgrade node option"
  ...
  # Don't need to convert nodenetwork if cloudweav-mgmt is used as vlan interface in any nodenetwork
  [[ $(kubectl get nodenetwork -o yaml | yq '.items[].spec.nic | select(. == "cloudweav-mgmt")') ]] &&
  echo "Don't need to convert nodenetwork because cloudweav-mgmt is used as VLAN interface in not less than one nodenetwork" && return

  local name=${CLOUDWEAV_UPGRADE_NODE_NAME}-vlan
  local EXIT_CODE=$?
  ...
  # the default ${CLOUDWEAV_UPGRADE_NODE_NAME}-vlan has no nics
  ...
  if [ "$type" == "bond" ]; then
    vlan_interface_details=$(v="$vlan_interface" yq '.install.networks.[env(v)]' $HOST_DIR/oem/cloudweav.config)
...
spec:
  clusterNetwork: vlan
  nodeSelector:
    kubernetes.io/hostname: "$CLOUDWEAV_UPGRADE_NODE_NAME"
  uplink:
    bondOptions:
      mode: $mode
      miimon: $miimon
...
set_reserved_resource() {
  ...
  local systemReserverConfig="$HOST_DIR/etc/rancher/rke2/config.yaml.d/99-z00-cloudweav-reserved-resources.yaml"
...
EOF
  ...
  # we would like to clean up the incomplete state.yaml to avoid the issue of https://github.com/cloudweav/cloudweav/issues/4526
  cleanup_incomplete_state_file
  ...
  multiPathEnabled=$(yq '.os.externalStorageConfig.enabled // false' ${HOST_DIR}/oem/cloudweav.config)
...
start_repo_vm() {
  repo_vm=$(kubectl get vm -l "cloudweavhci.io/upgrade=$CLOUDWEAV_UPGRADE_NAME" -n cloudweav-system -o yaml | yq -e e '.items[0].metadata.name' -)
  if [ -z $repo_vm ]; then
    echo "Fail to get upgrade repo VM name."
    exit 1
  fi

  virtctl start $repo_vm -n cloudweav-system || true
}
...
command_post_drain() {
  ...
  kubectl taint node $CLOUDWEAV_UPGRADE_NODE_NAME kubevirt.io/drain- || true
