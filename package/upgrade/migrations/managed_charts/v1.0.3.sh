...
{
  # Increase grafana pod limit and request (https://github.com/cloudweav/cloudweav-installer/pull/287)
...
{
  # enable alertmanager by default (https://github.com/cloudweav/cloudweav-installer/pull/322)
  ...
  yq e '.spec.values.alertmanager.alertmanagerSpec.storage.volumeClaimTemplate.spec.storageClassName = "cloudweav-longhorn"' $CHART_MANIFEST -i
...
{
if [ -n "$CLOUDWEAV_VIP" ]; then
  # enable alertmanager by default (https://github.com/cloudweav/cloudweav-installer/pull/322)
  PORT=9093
  yq e '.spec.values.alertmanager.service.port = '$PORT $CHART_MANIFEST -i
  yq e '.spec.values.alertmanager.alertmanagerSpec.externalUrl = "https://'$CLOUDWEAV_VIP'/api/v1/namespaces/cattle-monitoring-system/services/http:rancher-monitoring-alertmanager:'$PORT'/proxy/"' $CHART_MANIFEST -i
fi
}
...
if [ -n "$CLOUDWEAV_VIP" ]; then
  ...
  yq e '.spec.values.prometheus.prometheusSpec.externalUrl = "https://'$CLOUDWEAV_VIP'/api/v1/namespaces/cattle-monitoring-system/services/http:rancher-monitoring-prometheus:'$PORT'/proxy/"' $CHART_MANIFEST -i
fi
}

patch_ignoring_resource()
{
	# add ignoring resources when upgrading to match this pr (https://github.com/cloudweav/cloudweav-installer/pull/345)
...
# get cloudweav vip from service first, then configmap, skip potential error
get_cloudweav_vip()
{
  EXIT_CODE=0
