UPGRADE_NAMESPACE="cloudweav-system"
UPGRADE_REPO_URL=http://upgrade-repo-$CLOUDWEAV_UPGRADE_NAME.$UPGRADE_NAMESPACE/cloudweav-iso
UPGRADE_REPO_VM_NAME="upgrade-repo-$CLOUDWEAV_UPGRADE_NAME"
UPGRADE_REPO_RELEASE_FILE="$UPGRADE_REPO_URL/cloudweav-release.yaml"
...
detect_repo()
{
  ...
  REPO_CLOUDWEAV_VERSION=$(yq -e e '.cloudweav' $release_file)
  REPO_CLOUDWEAV_CHART_VERSION=$(yq -e e '.cloudweavChart' $release_file)
  REPO_OS_PRETTY_NAME="$(yq -e e '.os' $release_file)"
  REPO_OS_VERSION="${REPO_OS_PRETTY_NAME#Cloudweav }"
  ...
  REPO_CLOUDWEAV_MIN_UPGRADABLE_VERSION=$(yq e '.minUpgradableVersion' $release_file)

  if [ -z "$REPO_CLOUDWEAV_VERSION" ]; then
    echo "[ERROR] Fail to get Cloudweav version from upgrade repo."
    exit 1
  fi

  if [ -z "$REPO_CLOUDWEAV_CHART_VERSION" ]; then
    echo "[ERROR] Fail to get Cloudweav chart version from upgrade repo."
    exit 1
  fi
...
check_version()
{
  ...
  local ret=0
  upgrade-helper version-guard "$CLOUDWEAV_UPGRADE_NAME" || ret=$?
  if [ $ret -ne 0 ]; then
...
detect_upgrade()
{
  upgrade_obj=$(kubectl get upgrades.cloudweavhci.io $CLOUDWEAV_UPGRADE_NAME -n $UPGRADE_NAMESPACE -o yaml)

  UPGRADE_PREVIOUS_VERSION=$(echo "$upgrade_obj" | yq e .status.previousVersion -)
  SKIP_VERSION_CHECK=$(echo "$upgrade_obj" | yq e '.metadata.annotations."cloudweavhci.io/skip-version-check"' -)
}

# refer https://github.com/cloudweav/cloudweav/issues/3098
detect_node_current_cloudweav_version()
{
  NODE_CURRENT_CLOUDWEAV_VERSION=""
  local cloudweav_release_file=/host/etc/cloudweav-release.yaml

  if [ -f "$cloudweav_release_file" ]; then
    NODE_CURRENT_CLOUDWEAV_VERSION=$(yq e '.cloudweav' $cloudweav_release_file)
    echo "NODE_CURRENT_CLOUDWEAV_VERSION is: $NODE_CURRENT_CLOUDWEAV_VERSION"
  else
    echo "$cloudweav_release_file is not existing, NODE_CURRENT_CLOUDWEAV_VERSION is set as empty"
  fi
}
...
EOF
  ...
  item_count=$(kubectl get addons.cloudweavhci $name -n $namespace -o  jsonpath='{..name}' || true)
  if [ -z "$item_count" ]; then
    install_addon $name $namespace
  else
    kubectl patch addons.cloudweavhci $name -n $namespace --patch-file ./addon-patch.yaml --type merge
  fi
}
...
wait_for_addons_crd()
{
  item_count=$(kubectl get customresourcedefinitions addons.cloudweavhci.io -o  jsonpath='{.metadata.name}' || true)
  while [ -z "$item_count" ]
  do
    echo "wait for addons.cloudweavhci.io crd to be created"
    sleep 10
    item_count=$(kubectl get customresourcedefinitions addons.cloudweavhci.io -o  jsonpath='{.metadata.name}' || true)
  done
}
...
# detect cloudweav vip from service first, then configmap, skip potential error
detect_cloudweav_vip()
{
  local EXIT_CODE=0
...
upgrade_addon_try_patch_version_only()
{
  ...
  local version=$(kubectl get addons.cloudweavhci.io $name -n $namespace -o=jsonpath='{.spec.version}' || true)
  if [[ -z "$version" ]]; then
...
EOF
  ...
  local enabled=""
  local curstatus=""
  enabled=$(kubectl get addons.cloudweavhci.io $name -n $namespace -o=jsonpath='{.spec.enabled}' || true)
  if [[ $enabled = "true" ]]; then
    curstatus=$(kubectl get addons.cloudweavhci.io $name -n $namespace -o=jsonpath='{.status.status}' || true)
  fi

  kubectl patch addons.cloudweavhci.io $name -n $namespace --patch-file ./$patchfile --type merge
  rm -f ./$patchfile
  ...
  if [[ $enabled = "true" ]]; then
    if [[ "$curstatus" = "$targetstatus" ]]; then
      ...
      do
        sleep 5
        local status=""
        status=$(kubectl get addons.cloudweavhci.io $name -n $namespace -o=jsonpath='{.status.status}' || true)
...
upgrade_addon_rancher_logging_with_patch_eventrouter_image()
{
  ...
  rm -f $valuesfile
  kubectl get addons.cloudweavhci.io $name -n $namespace -ojsonpath="{.spec.valuesContent}" > $valuesfile
  ...
  if [ $EXIT_CODE != 0 ]; then
    ...
    if [[ "panmeta/cloudweav-eventrouter:v0.3.2" > $tag ]]; then
  ...
  if [[ $fixeventrouter == true ]]; then
    yq -e '.eventTailer.workloadOverrides.containers[0].image = "panmeta/cloudweav-eventrouter:v0.3.2"' -i $valuesfile
  fi
...
upgrade_nvidia_driver_toolkit_addon()
{
  ...
  CURRENTENDPOINT=$(kubectl get addons.cloudweav nvidia-driver-toolkit -n cloudweav-system -o yaml | yq .spec.valuesContent | yq '.driverLocation // "empty"')
  ...
  upgrade_addon nvidia-driver-toolkit cloudweav-system
}
