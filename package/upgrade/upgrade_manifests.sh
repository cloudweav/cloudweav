...
wait_longhorn_manager() {
  ...
  lm_repo=$(kubectl get apps.catalog.cattle.io/cloudweav -n cloudweav-system -o json | jq -r .spec.chart.values.longhorn.image.longhorn.manager.repository)
  lm_tag=$(kubectl get apps.catalog.cattle.io/cloudweav -n cloudweav-system -o json | jq -r .spec.chart.values.longhorn.image.longhorn.manager.tag)
  lm_image="${lm_repo}:${lm_tag}"
  node_count=$(kubectl get nodes --selector=cloudweavhci.io/managed=true,node-role.cloudweavhci.io/witness!=true -o json | jq -r '.items | length')
...
wait_longhorn_instance_manager_aio() {
  ...
  im_repo=$(kubectl get apps.catalog.cattle.io/cloudweav -n cloudweav-system -o json | jq -r .spec.chart.values.longhorn.image.longhorn.instanceManager.repository)
  im_tag=$(kubectl get apps.catalog.cattle.io/cloudweav -n cloudweav-system -o json | jq -r .spec.chart.values.longhorn.image.longhorn.instanceManager.tag)
...
get_cluster_repo_index_download_time() {
  local output_type=$1
  local iso_time=$(kubectl get clusterrepos.catalog.cattle.io cloudweav-charts -ojsonpath='{.status.downloadTime}')
...
upgrade_rancher() {
  ...
  # drop the potential manual patch upon shell-image to v0.1.26 on Cloudweav v1.3.2
...
upgrade_cloudweav_cluster_repo() {
  echo "Upgrading Cloudweav Cluster Repo"

  mkdir -p $UPGRADE_TMP_DIR/cloudweav_cluster_repo
  cd $UPGRADE_TMP_DIR/cloudweav_cluster_repo
...
spec:
  template:
    spec:
      containers:
        - name: httpd
          image: panmeta/cloudweav-cluster-repo:$REPO_OS_VERSION
EOF
  kubectl patch deployment cloudweav-cluster-repo -n cattle-system --patch-file ./cluster_repo.yaml --type strategic

  until kubectl -n cattle-system rollout status -w deployment/cloudweav-cluster-repo; do
    echo "Waiting for cloudweav-cluster-repo deployment ready..."
    sleep 5
  done
...
EOF
  kubectl patch clusterrepos.catalog.cattle.io cloudweav-charts --patch-file ./catalog_cluster_repo.yaml --type merge
...
modify_nad_bridge() {
  ...
  [[ $(kubectl get nodenetwork -o yaml | yq '.items[].spec.nic | select(. == "cloudweav-mgmt")') ]] && bridge="mgmt-br"
...
patch_longhorn_settings() {
  # set Longhorn default settings with Cloudweav expected values, only when they are not set in managedchart and LH uses the default value
  local target=$1
  local EXIT_CODE=0
...
# but Cloudweav setting additional-guest-memory-overhead-ratio is not existing
# try to convert it to Cloudweav setting to keep the configured value
convert_kubevirt_additional_guest_memory_overhead_to_cloudweav_setting() {
  local settingfile="additional-guest-memory-overhead-ratio.yaml"
  local settingname="additional-guest-memory-overhead-ratio"
  local agmor=$(kubectl get kubevirt kubevirt -n cloudweav-system -ojsonpath="{.spec.configuration.additionalGuestMemoryOverheadRatio}")
  local setting=$(kubectl get settings.cloudweavhci.io "$settingname")

  if [[ -n "$agmor" ]]; then
    echo "kubevirt additionalGuestMemoryOverheadRatio setting is $agmor"
    if [[ -z $setting ]]; then
      echo "try to convert kubevirt additionalGuestMemoryOverheadRatio to cloudweav setting $settingname"
...
EOF
      ...
      # the apply may be denied by cloudweav webhook
      kubectl apply -f $settingfile || echo "failed to create setting $settingname, skip"
      echo "the created object yaml"
      kubectl get settings.cloudweavhci.io $settingname -oyaml ||  echo "failed to get setting $settingname"
      rm -f $settingfile
    else
      echo "cloudweav setting $settingname is found, do not re-convert"
    fi
  else
...
upgrade_cloudweav() {
  echo "Upgrading Cloudweav"

  # after v1.4 is released, this function may be dropped from master-head
  convert_kubevirt_additional_guest_memory_overhead_to_cloudweav_setting

  pre_generation_cloudweav=$(kubectl get managedcharts.management.cattle.io cloudweav -n fleet-local -o=jsonpath='{.status.observedGeneration}')
  pre_generation_cloudweav_crd=$(kubectl get managedcharts.management.cattle.io cloudweav-crd -n fleet-local -o=jsonpath='{.status.observedGeneration}')

  mkdir -p $UPGRADE_TMP_DIR/cloudweav
  cd $UPGRADE_TMP_DIR/cloudweav

  cat >cloudweav-crd.yaml <<EOF
spec:
  version: $REPO_CLOUDWEAV_CHART_VERSION
  timeoutSeconds: 60
EOF
  kubectl patch managedcharts.management.cattle.io cloudweav-crd -n fleet-local --patch-file ./cloudweav-crd.yaml --type merge

  cat >cloudweav.yaml <<EOF
...
EOF
  kubectl get managedcharts.management.cattle.io -n fleet-local cloudweav -o yaml | yq e '{"spec": .spec}' - >>cloudweav.yaml

  upgrade_managed_chart_from_version $UPGRADE_PREVIOUS_VERSION cloudweav cloudweav.yaml
  NEW_VERSION=$REPO_CLOUDWEAV_CHART_VERSION yq e '.spec.version = strenv(NEW_VERSION)' cloudweav.yaml -i

  local sc=$(kubectl get sc -o json | jq '.items[] | select(.metadata.annotations."storageclass.kubernetes.io/is-default-class" == "true" and .metadata.name != "cloudweav-longhorn")')
  if [ -n "$sc" ] && [ "$UPGRADE_PREVIOUS_VERSION" != "v1.0.3" ]; then
      yq e '.spec.values.storageClass.defaultStorageClass = false' -i cloudweav.yaml
  fi

  patch_longhorn_settings cloudweav.yaml

  # set timeoutSeconds to 10 minutes to avoid context deadline exceeded in post upgrade hooks
  yq eval '.spec.timeoutSeconds = 600' -i cloudweav.yaml

  kubectl apply -f ./cloudweav.yaml

  pause_managed_chart cloudweav "false"
  pause_managed_chart cloudweav-crd "false"

  wait_managed_chart fleet-local cloudweav $REPO_CLOUDWEAV_CHART_VERSION $pre_generation_cloudweav ready
  wait_managed_chart fleet-local cloudweav-crd $REPO_CLOUDWEAV_CHART_VERSION $pre_generation_cloudweav_crd ready

  wait_kubevirt cloudweav-system kubevirt $REPO_KUBEVIRT_VERSION
}
...
pause_all_charts() {
  local charts="cloudweav cloudweav-crd rancher-monitoring-crd rancher-logging-crd"
...
skip_restart_rancher_system_agent() {
  ...
  plan_name="$CLOUDWEAV_UPGRADE_NAME"-skip-restart-rancher-system-agent
...
{
  wait_for_addons_crd
  addons="vm-import-controller pcidevices-controller cloudweav-seeder"
  for addon in $addons; do
    upgrade_addon $addon "cloudweav-system"
  done

  # those 2 addons have flexible user-configurable fields, only upgrade cloudweav related e.g. new image tag
...
sync_containerd_registry_to_rancher() {
  ...
  kubectl annotate --overwrite=true setting.cloudweavhci.io containerd-registry "cloudweavhci.io/upgrade-patched=$REPO_CLOUDWEAV_VERSION"
}
