...
var (
	...
	crdList          = []string{"./manifest/helm-crd.yaml", "./manifest/app-crd.yaml", "./manifest/ranchersettings-crd.yaml", "./manifest/clusterrepos-crd.yaml", "../../../deploy/charts/cloudweav-crd/templates/cloudweavhci.io_addons.yaml"}
)
...
var _ = ginkgo.BeforeSuite(func() {
	...
	err = cloudweavv1.AddToScheme(scheme)
	dsl.MustNotError(err)
...
var _ = ginkgo.AfterSuite(func() {
	...
	ginkgo.By("tearing down cloudweav server")
...
func startControllers(ctx context.Context, restConfig *rest.Config, opts *ctlcloudweavv1.FactoryOptions) error {

	// to speed up testing, override default backofflimit for jobs
	cloudweavv1.DefaultJobBackOffLimit = 1
	...
	if err = start.All(ctx, 10, cloudweavFactory, core, batch, helm, catalog, rancher); err != nil {
		return err
	}
