...
import (
	...
	"github.com/cloudweav/cloudweav/tests/integration/controllers/fake"
)

var _ = ginkgo.Describe("verify helm chart is create and addon gets to desired state", func() {

	var a *cloudweavv1.Addon
	var app *catalogv1.App
	var addonController ctlcloudweavv1.AddonController
	...
	ginkgo.BeforeEach(func() {
		a = &cloudweavv1.Addon{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					"cloudweavhci.io/addon-defaults": "ZGVmYXVsdFZhbHVlcwo=",
				},
			},
			Spec: cloudweavv1.AddonSpec{
				Chart:   "vm-import-controller",
				Repo:    "http://cloudweav-cluster-repo.cattle-system.svc",
				Version: "v0.1.0",
				Enabled: true,
			},
		}
		...
		gomega.Eventually(func() error {
			addonController = scaled.Management.CloudweavFactory.Cloudweavhci().V1beta1().Addon()
	...
	ginkgo.It("checking helm and addon reconcile", func() {
		...
		ginkgo.By("check status of addon", func() {
			gomega.Eventually(func() error {
				...
				if aObj.Status.Status != cloudweavv1.AddonDeployed {
	...
	ginkgo.It("verify helm deletion tasks", func() {
		...
		ginkgo.By("check addon status is successful", func() {
			gomega.Eventually(func() error {
				...
				if a.Status.Status != cloudweavv1.AddonDeployed {
	...
	ginkgo.It("reconcile helm chart upgrade", func() {
		...
		ginkgo.By("update addon", func() {
			gomega.Eventually(func() error {
				...
				aObj.Spec.Repo = "http://cloudweav-cluster-repo.cattle-system.svc.cluster.local"
	...
	ginkgo.It("checking helm and addon reconcile", func() {
		...
		ginkgo.By("check status of addon", func() {
			gomega.Eventually(func() error {
				...
				if aObj.Status.Status == cloudweavv1.AddonEnabling && cloudweavv1.AddonOperationFailed.IsTrue(aObj) {
					return nil
				}
	...
	ginkgo.BeforeEach(func() {
		a = &cloudweavv1.Addon{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					"cloudweavhci.io/addon-defaults":          "ZGVmYXVsdFZhbHVlcwo=",
					"cloudweavhci.io/addon-operation-timeout": "1",
				},
			},
			Spec: cloudweavv1.AddonSpec{
				Chart:   "vm-import-controller",
	...
	ginkgo.It("check disable addon", func() {
		...
		ginkgo.By("updating addon to disable", func() {
			gomega.Eventually(func() error {
				...
				a.Annotations["cloudweavhci.io/addon-last-operation"] = "disable"
				a.Annotations["cloudweavhci.io/addon-last-operation-timestamp"] = time.Now().UTC().Format(time.RFC3339)
				a.Spec.Enabled = false
				_, err = addonController.Update(a)
				return err
			}, "30s", "5s").ShouldNot(gomega.HaveOccurred())
	...
	ginkgo.It("reconcile helm chart upgrade", func() {
		...
		ginkgo.By("check addon upgrade failed", func() {
			gomega.Eventually(func() error {
				...
				if aObj.Status.Status == cloudweavv1.AddonUpdating && cloudweavv1.AddonOperationFailed.IsTrue(aObj) {
					return nil
				}
