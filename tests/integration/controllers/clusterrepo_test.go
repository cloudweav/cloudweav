...
var _ = ginkgo.Describe("verify cluster repos are patched", func() {
	var cloudweavcharts, partnercharts *catalogv1.ClusterRepo
	var clusterRepoController ctlcatalogv1.ClusterRepoController

	ginkgo.BeforeEach(func() {
		cloudweavcharts = &catalogv1.ClusterRepo{
			ObjectMeta: metav1.ObjectMeta{
				Name: "cloudweav-charts",
			},
		...
		gomega.Eventually(func() error {
			_, err := clusterRepoController.Create(cloudweavcharts)
			return err
	...
	ginkgo.It("verify cluster repo annotations", func() {
		ginkgo.By("checking annotation on cloudweav-charts", func() {
			gomega.Eventually(func() error {
				obj, err := clusterRepoController.Get(cloudweavcharts.Name, metav1.GetOptions{})
				if err != nil {
					return err
				}
	...
	ginkgo.AfterEach(func() {
		gomega.Eventually(func() error {
			return clusterRepoController.Delete(cloudweavcharts.Name, &metav1.DeleteOptions{})
		}, "30s", "5s").ShouldNot(gomega.HaveOccurred())
