...
import (
	...
	apivm "github.com/cloudweav/cloudweav/pkg/api/vm"
...
var _ = Describe("verify vm backup & restore APIs", func() {
	if env.IsE2ETestsEnabled() {
		var (
			...
			backupController  ctlcloudweavv1.VirtualMachineBackupController
		...
		BeforeEach(func() {
			...
			backupController = scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineBackup()
			restoreController = scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineRestore()
			...
			vmBackupTarget = fmt.Sprintf("nfs://cloudweav-nfs-svc.%s:/opt/backupstore", backupNamespace)
		})
		...
		Context("operate via steve API", func() {
			...
			BeforeEach(func() {
				...
				restoresAPI = helper.BuildAPIURL("v1", "cloudweavhci.io.virtualmachinerestores", options.HTTPSListenPort)
			})
			...
			Specify("verify vm backup api", func() {
				...
				By("then validate restore an new vm", func() {
					...
					newVM := cloudweavv1.VirtualMachineRestore{
...
func createLonghornTestingNFS(svcController ctlcorev1.ServiceController, podController ctlcorev1.PodController, namespace string) (string, error) {
	testNFSServerLabels := map[string]string{
		"cloudweav.test.io/name": "cloudweav-nfs",
	}
	name := "cloudweav-nfs"
	privileged := true
