...
var _ = Describe("verify vm template APIs", func() {
	...
	BeforeEach(func() {
		...
		templates = scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineTemplate()
		templateVersions = scaled.CloudweavFactory.Cloudweavhci().V1beta1().VirtualMachineTemplateVersion()
	...
	Context("operate via steve API", func() {
		...
		BeforeEach(func() {
			...
			templateAPI = helper.BuildAPIURL("v1", "cloudweavhci.io.virtualmachinetemplates", port)
			templateVersionAPI = helper.BuildAPIURL("v1", "cloudweavhci.io.virtualmachinetemplateversions", port)
		...
		Specify("verify the vm template and template versions", func() {
			...
			By("create a vm template version", func() {
				...
				MustNotError(err)
				templateVersion.Spec.VM = cloudweavv1.VirtualMachineSourceSpec{
