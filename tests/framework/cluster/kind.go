...
import (
	...
	"github.com/cloudweav/cloudweav/tests/framework/finder"
	"github.com/cloudweav/cloudweav/tests/framework/fuzz"
	"github.com/cloudweav/cloudweav/tests/framework/logs"
)
...
`
	...
	tp, err := template.New("cloudweav").Funcs(tpFuncMap).Parse(tpText)
	if err != nil {
...
func NewLocalKindCluster() *LocalKindCluster {
	localKindClusterOnce.Do(func() {
		...
		localKindCluster = &LocalKindCluster{
			...
			ClusterName:            envFinder.Get("clusterName", "cloudweav"),
