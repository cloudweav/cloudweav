...
const (
	...
	// Specify whether to skip Cloudweav installation
	// default is to drop it.
	envSkipCloudweavInstallation = "SKIP_CLOUDWEAV_INSTALLATION"

	// Specify to keep the testing runtime to review,
	// default is to drop it.
	envKeepCloudweavInstallation = "KEEP_CLOUDWEAV_INSTALLATION"
	...
	// If you use an existing cloudweav runtime to test , you can configure as follows:
	// export USE_EXISTING_CLUSTER=true
	// export SKIP_CLOUDWEAV_INSTALLATION=true
...
// IsSkipCloudweavInstallation validates whether to skip cloudweav installation for testing,
// if "SKIP_CLOUDWEAV_INSTALLATION=true", will not create or update the cloudweav chart for testing.
func IsSkipCloudweavInstallation() bool {
	return IsTrue(envSkipCloudweavInstallation)
}

// IsKeepingCloudweavInstallation validates whether keep the cloudweav installation,
// if "KEEP_CLOUDWEAV_INSTALLATION=true", will keep the cloudweav installation for reviewing.
func IsKeepingCloudweavInstallation() bool {
	return IsTrue(envKeepCloudweavInstallation)
}
